...
import { useApiServerStore, type TokenAuditEntry } from "../../store/apiServerStore";
import { useT } from "../../lib/i18n";
import { buildWidgetEmbedSnippet } from "../../lib/chatWidgetEmbed";
...
/**
 * Settings "API Server" compatibility tab: on/off toggle, port field,
 * legacy-token inventory/revocation, and widget configuration. New clients
 * deliberately cannot mint a legacy token here; the visible primary action
 * sends them to Runtime Hub's scoped pairing flow. Existing tokens remain
 * manageable because published Local Apps embed restricted compatibility
 * tokens that must keep working throughout the release migration.
 */
export function ApiServerPanel({ onOpenRuntimeHubPairing }: { onOpenRuntimeHubPairing: () => void }) {
  const { t } = useT();
  ...
  return (
    <div className="flex flex-col gap-4 py-2">
      ...
      <section
        aria-labelledby="api-server-pairing-migration-heading"
        className="flex flex-col gap-2 rounded-lg border border-accent/40 bg-accent-soft p-3"
      >
        <div>
          <h3 id="api-server-pairing-migration-heading" className="text-sm font-semibold text-foreground">
            {t("ApiServerPanel.pairingMigrationHeading")}
          </h3>
          <p className="mt-1 text-xs leading-5 text-muted">
            {t("ApiServerPanel.pairingMigrationDescription")}
          </p>
        </div>
        <div>
          <Button variant="primary" size="sm" onClick={onOpenRuntimeHubPairing}>
            {t("ApiServerPanel.pairingMigrationButton")}
          </Button>
        </div>
      </section>
      ...
      <section>
        ...
        <p className="mb-2 text-xs leading-5 text-faint">{t("ApiServerPanel.legacyTokensDescription")}</p>
