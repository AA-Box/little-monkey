...
function ConfigFieldInput({
  ...
  return (
    <label className="text-xs text-muted">
      ...
      {field.hint && (
        <span className="mt-1 block text-faint">{field.hint}</span>
      )}
    </label>
  );
}
...
export function ChannelsPanel() {
  ...
  const load = useCallback(async () => {
    try {
      const [listed, routes] = await Promise.all([
        channelsList(),
        channelsRoutes(),
      ]);
  ...
  const loadDetail = useCallback(async (accountId: string, kind: string) => {
    try {
      const [waiting, recent] = await Promise.all([
        channelsSenders(accountId),
        channelsEvents(accountId, 20),
      ]);
      ...
      setCallback(
        needsPublicCallback(kind) ? await channelsCallbackUrl(accountId) : null,
      );
    } catch (reason) {
      setError(errorMessage(reason));
    }
  ...
  const selectedKind =
    accounts?.find((entry) => entry.account_id === selected)?.kind ?? null;
  useEffect(() => {
    setSecret("");
    setSecretParts({});
  ...
  const account =
    accounts?.find((entry) => entry.account_id === selected) ?? null;
  const guide = PROVIDER_GUIDES.find(
    (entry) => entry.kind === (account?.kind ?? draft.kind),
  );
  const draftGuide = PROVIDER_GUIDES.find((entry) => entry.kind === draft.kind);
  const missingConfig = missingRequiredConfig(
    draftGuide?.configFields ?? [],
    configDraft,
  );
  ...
  const credentialValue =
    fields.length > 0
      ? JSON.stringify(
          Object.fromEntries(
            fields.map((field) => [field.key, secretParts[field.key] ?? ""]),
          ),
        )
      : secret;
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {notice && (
        <p className="rounded-md border border-border bg-background p-2 text-xs text-muted">
          {notice}
        </p>
      )}
            ...
            {accounts.map((entry) => (
              <li key={entry.account_id}>
                <button
                  ...
                  onClick={() =>
                    setSelected(
                      entry.account_id === selected ? null : entry.account_id,
                    )
                  }
                ...
                >
                  ...
                  <span className={`text-xs ${healthTone(entry.health)}`}>
                    {entry.enabled
                      ? t(`ChannelsPanel.health_${entry.health}`)
                      : t("ChannelsPanel.disabled")}
                  </span>
                </button>
              </li>
            ))}
      ...
      {account && (
        <section className="rounded-lg border border-border bg-surface p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-muted">
                {t("ChannelsPanel.transport")}:{" "}
                {guide
                  ? t(`ChannelsPanel.transport_${guide.transport}`)
                  : account.kind}
              </p>
              ...
              {account.last_error && (
                <p className="mt-1 text-xs text-danger">{account.last_error}</p>
              )}
              {account.credential_required && !account.has_credential && (
                <p className="mt-1 text-xs text-warning">
                  {t("ChannelsPanel.needsCredential")}
                </p>
              )}
              ...
              {account.callback_rejections.count > 0 && (
                <p className="mt-2 flex items-start gap-2 rounded-md border border-danger/40 bg-danger/10 p-2 text-xs text-danger">
                  <AlertTriangle size={14} className="mt-0.5 shrink-0" />
                  <span>
                    {t("ChannelsPanel.callbacksRejected", {
              ...
              {settingsDraft === null &&
                Object.keys(account.non_secret_config).length > 0 && (
                  <dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 text-xs text-faint">
                    {Object.entries(account.non_secret_config).map(
                      ([key, value]) => (
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                disabled={busy !== null}
                onClick={() =>
                  void run("probe", () => channelsProbe(account.account_id))
                }
              >
                {busy === "probe" ? (
                  <Loader2 size={14} className="animate-spin" />
                ...
                {t("ChannelsPanel.testConnection")}
              </Button>
              <Button
                size="sm"
                disabled={busy !== null}
              ...
              <Button
                ...
                onClick={() =>
                  void run("enable", () =>
                    channelsEnable(account.account_id, !account.enabled),
                  )
                }
              >
                <Power size={14} />
                {account.enabled
                  ? t("ChannelsPanel.disable")
                  : t("ChannelsPanel.enable")}
              </Button>
              <Button
                size="sm"
                variant="danger"
                ...
                onClick={() =>
                  void run(
                    "remove",
                    () => channelsRemove(account.account_id),
                    t("ChannelsPanel.removed"),
                  )
                }
              >
                <Trash2 size={14} />
          ...
          <div className="mt-3 flex flex-col gap-2 border-t border-border pt-3 sm:flex-row sm:items-end">
            {!account.credential_required ? (
              ...
              <p className="min-w-0 flex-1 text-xs text-muted">
                {t("ChannelsPanel.noCredentialNeeded")}
              </p>
            ) : fields.length > 0 ? (
              <div className="grid min-w-0 flex-1 gap-2 sm:grid-cols-2">
                {fields.map((field) => (
                  <label key={field.key} className="min-w-0 text-xs text-muted">
                    {field.label}
                    <input
                      className={`${INPUT} mt-1`}
                      type="password"
              ...
              <label className="min-w-0 flex-1 text-xs text-muted">
                {guide?.credentialLabel ?? t("ChannelsPanel.credential")}
                <input
                  className={`${INPUT} mt-1`}
                  type="password"
                  value={secret}
            ...
            {account.credential_required && (
              ...
              >
                <KeyRound size={14} />
                {t("ChannelsPanel.saveCredential")}
              </Button>
            )}
          </div>
          {guide && (
            <p className="mt-2 text-xs text-faint">
              ...
              >
                {t("ChannelsPanel.setupInstructions")}
          ...
          {settingsDraft !== null && (
            <div className="mt-3 border-t border-border pt-3">
              <h5 className="text-xs font-semibold">
                {t("ChannelsPanel.settings")}
              </h5>
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
                <label className="text-xs text-muted">
                  {t("ChannelsPanel.label")}
                  <input
                    className={`${INPUT} mt-1`}
                    value={labelDraft}
                ...
                {(guide?.configFields ?? []).map((field) => (
                  <ConfigFieldInput
                    ...
                    onChange={(next) =>
                      setSettingsDraft({ ...settingsDraft, [field.key]: next })
                    }
              ...
              <h6 className="mt-3 text-xs font-semibold text-muted">
                {t("ChannelsPanel.advancedSettings")}
              </h6>
              <div className="mt-2 grid gap-2 sm:grid-cols-2">
              ...
              <p className="mt-2 text-xs text-faint">
                {t("ChannelsPanel.settingsReprobeHint")}
              </p>
              <Button
                className="mt-2"
                size="sm"
              ...
              {callback?.configured && callback.url ? (
                <>
                  ...
                  <p className="mt-1 flex flex-wrap items-center gap-2">
                    ...
                    <Button
                      size="sm"
                      onClick={() =>
                        void navigator.clipboard.writeText(callback.url ?? "")
                      }
                    >
                      <Copy size={12} />
                      {t("ChannelsPanel.copy")}
                    </Button>
                  </p>
                </>
              ) : (
                <>
                  <p className="text-warning">
                    {t("ChannelsPanel.callbackUnconfigured")}
                  </p>
                  <p className="mt-1">
                    {t("ChannelsPanel.callbackPathIs")}{" "}
                  ...
                  <div className="mt-2 flex flex-col gap-2 sm:flex-row sm:items-end">
                    ...
                    <Button
                      size="sm"
                      disabled={
                        busy !== null || publicUrlDraft.trim().length === 0
                      }
                      onClick={() =>
                        void run(
                          "public-url",
                    ...
                    >
                      {t("ChannelsPanel.savePublicBaseUrl")}
                    </Button>
                  </div>
                  <p className="mt-1 text-faint">
                    {t("ChannelsPanel.publicBaseUrlHint")}
                  </p>
                </>
              )}
              {account.kind === "whatsapp" && (
                <p className="mt-1">{t("ChannelsPanel.callbackVerifyHint")}</p>
              )}
            </div>
          )}
              ...
              <select
                ...
                onChange={(event) =>
                  void run("policy", () =>
                    channelsSetPolicy(
                      account.account_id,
                      event.target.value as AccessPolicy,
                      null,
                      null,
                    ),
                  )
              ...
              >
                {(
                  ...
                  <option key={value} value={value}>
                    {t(`ChannelsPanel.policy_${value}`)}
                  </option>
                ))}
              </select>
            </label>
            <label className="text-xs text-muted">
              {t("ChannelsPanel.groupPolicy")}
              <select
                className={`${INPUT} mt-1`}
                ...
                onChange={(event) =>
                  void run("policy", () =>
                    channelsSetPolicy(
                      ...
                      null,
                      null,
                      event.target.value as GroupActivation,
                    ),
                  )
                }
              >
                {(
                  ["mention_only", "always", "disabled"] as GroupActivation[]
                ).map((value) => (
                  <option key={value} value={value}>
                    {t(`ChannelsPanel.activation_${value}`)}
                  </option>
                ))}
              </select>
            </label>
          ...
          <div className="mt-3 border-t border-border pt-3">
            <h5 className="text-xs font-semibold">
              {t("ChannelsPanel.pendingSenders")}
            </h5>
            {senders.length === 0 ? (
              <p className="mt-1 text-xs text-faint">
                {t("ChannelsPanel.noPendingSenders")}
              </p>
            ) : (
                      ...
                      <Button
                        ...
                        onClick={() =>
                          void run(
                            ...
                            t("ChannelsPanel.senderApproved"),
                          )
                        }
                      >
                        <Check size={12} />
                        {t("ChannelsPanel.approve")}
                      </Button>
                      <Button
                        size="sm"
                        variant="danger"
          ...
          <div className="mt-3 border-t border-border pt-3">
            <div className="flex items-center justify-between">
              <h5 className="text-xs font-semibold">
                {t("ChannelsPanel.activity")}
              </h5>
              <Button
                size="sm"
                disabled={busy !== null}
                onClick={() =>
                  void loadDetail(account.account_id, account.kind)
                }
              >
                <RefreshCw size={12} />
                {t("ChannelsPanel.refresh")}
              </Button>
            </div>
            {events.length === 0 ? (
              <p className="mt-1 text-xs text-faint">
                {t("ChannelsPanel.noActivity")}
              </p>
            ) : (
              <ul className="mt-2 flex flex-col gap-1">
                {events.map((event) => (
                  ...
                  >
                    <span className="truncate">
                      {event.direction} · {event.conversation_id}
                    </span>
                    <span className="text-faint">
                      {event.ignore_reason ?? event.disposition}
                    </span>
                  </li>
                ))}
              </ul>
      ...
      <section className="rounded-lg border border-border bg-surface p-4">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          <label className="text-xs text-muted">
            ...
            >
              {PROVIDER_GUIDES.filter((entry) => !entry.editOnly).map(
                (entry) => (
          ...
          <label className="text-xs text-muted">
            ...
            <input
              ...
              placeholder={t("ChannelsPanel.labelPlaceholder")}
            />
          </label>
          {(draftGuide?.configFields ?? []).map((field) => (
            <ConfigFieldInput
              ...
              onChange={(next) =>
                setConfigDraft({ ...configDraft, [field.key]: next })
              }
              yesLabel={t("ChannelsPanel.yes")}
              noLabel={t("ChannelsPanel.no")}
            />
        ...
        {draftGuide?.requiresPlatform === "macos" && (
          <p className="mt-2 text-xs text-warning">
            {t("ChannelsPanel.macOnly")}
          </p>
        )}
        {missingConfig.length > 0 && (
          <p className="mt-2 text-xs text-faint">
            {t("ChannelsPanel.missingSettings")} {missingConfig.join(", ")}
          </p>
        )}
