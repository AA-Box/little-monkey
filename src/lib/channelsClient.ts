...
export const PROVIDER_GUIDES: ProviderGuide[] = [
  {
    ...
    whereToGetIt:
      "Create a bot with @BotFather and copy the token it gives you.",
  ...
  },
  {
    ...
    credentialLabel: "Bot token",
    whereToGetIt:
      "Discord Developer Portal → your application → Bot → Reset Token. Enable the Message Content intent.",
  ...
  },
  {
    ...
    label: "Slack",
    transport: "socket",
    credentialLabel: "Bot and app tokens (JSON)",
    whereToGetIt:
      "Slack API → your app → OAuth (xoxb bot token) and Basic Information (xapp app-level token with connections:write).",
  ...
  },
  {
    ...
    whereToGetIt:
      "Your Mattermost profile → Security → Personal Access Tokens. Messages arrive over a WebSocket, so health reports the connection as well as the token: an account whose token works but whose socket is down is shown as degraded rather than connected, and so is one dropping posts because replies are running behind.",
    docsUrl:
    ...
    configFields: [
      {
        ...
        hint: "Your own server's origin. Plain http is accepted only for localhost, so a token can never be walked to an unencrypted host.",
      },
    ],
  },
  {
    ...
    label: "IRC",
    transport: "socket",
    credentialLabel: "SASL password",
    credentialOptional: true,
    whereToGetIt:
      "The account password registered with the network's services (NickServ). Only needed if you turn SASL on. If the nickname you ask for is already in use, the connection takes the next free one (littlemonkey_, littlemonkey_2, \u2026) and health shows which one it ended up with. With SASL on, a failed login stays a failure: the connection is never completed as an anonymous one, and health names the account that could not authenticate.",
    ...
    configFields: [
      ...
      },
      {
        ...
        type: "number",
        placeholder: "6697",
        hint: "TLS is always used; 6697 is the usual TLS port.",
      },
      {
        key: "nick",
        label: "Nickname",
        type: "text",
        required: true,
        placeholder: "littlemonkey",
      },
      {
        key: "channels",
        label: "Channels to join",
        type: "list",
        placeholder: "#room-one, #room-two",
      },
      { key: "use_sasl", label: "Log in with SASL", type: "boolean" },
      {
        key: "sasl_username",
        label: "SASL account (optional)",
        type: "text",
        placeholder: "littlemonkey",
        hint: "The account name registered with the network's services, when it differs from the nickname. Leave empty to authenticate as the nickname. If the nickname is taken, the connection takes the next free one \u2014 but always authenticates as this account.",
      },
    ],
  },
  {
    ...
    whereToGetIt:
      "Your own homeserver account's access token \u2014 in Element: Settings \u2192 Help & About \u2192 Advanced \u2192 Access Token. Treat it like a password. Encrypted rooms, encrypted files and threads all work: this app appears as the device that token already belongs to rather than adding a new one, and it keeps that device across restarts. Messages sent before it joined stay unreadable until you verify it from another of your clients. If it ever cannot tell whether a room is encrypted, it refuses to send rather than risk sending in the clear.",
    ...
    configFields: [
      ...
      },
      {
        ...
        hint: "The session this access token belongs to, as your other Matrix client lists it. Leave empty to ask the homeserver \u2014 either way, no new device is ever registered.",
      },
    ],
  },
  {
    ...
    label: "Signal",
    transport: "helper",
    credentialLabel: "None — signal-cli holds the account",
    credentialOptional: true,
    whereToGetIt:
      "Install signal-cli yourself and register or link your own number with it. Little Monkey never bundles, downloads or installs it, and never reads its account store. Health checks that this number is actually registered with the helper, not just that the helper starts.",
    ...
    configFields: [
      ...
      },
      {
        ...
        hint: "The number signal-cli is registered or linked as.",
      },
    ],
  },
  {
    ...
    configFields: [
      ...
      },
      {
        ...
        type: "text",
        required: true,
        placeholder: "/usr/local/bin/little-monkey-imessage-helper",
        hint: "Where you installed the helper. Health reports which permission is still missing if either grant has not been made yet.",
      },
    ],
  },
  {
    ...
    label: "WhatsApp",
    transport: "webhook",
    credentialLabel: "WhatsApp credentials",
    whereToGetIt:
  ...
  },
  {
    ...
    label: "LINE",
    transport: "webhook",
    credentialLabel: "LINE credentials",
    whereToGetIt:
      "LINE Developers Console → your channel → Messaging API for the access token, and Basic settings for the channel secret that verifies signatures. Paste the callback URL below into Webhook URL, then turn Use webhook on and press Verify.",
  ...
  },
  {
    ...
    whereToGetIt:
      "Azure Bot resource → Configuration for the Microsoft App ID and tenant ID; Certificates & secrets for a client secret. Paste the callback URL below into that same Configuration page as the Messaging endpoint. The public Bot Framework cloud is the one supported: a bot in a sovereign cloud will not authenticate here.",
    docsUrl: "https://learn.microsoft.com/azure/bot-service/",
    configFields: [
      {
        key: "app_id",
    ...
    secretFields: [{ key: "app_password", label: "Client secret" }],
  },
  {
    kind: "google_chat",
    label: "Google Chat",
    transport: "webhook",
    credentialLabel: "Service account key (paste the whole JSON file)",
    whereToGetIt:
      "Google Cloud Console → Chat API → create a service account and download its key. Paste the file's contents unchanged. Then on the same API's Configuration page set exactly these two, or deliveries are refused — Connection settings: HTTP endpoint URL (paste the callback URL below into it), and Authentication Audience: Project Number. App URL is the other Authentication Audience value and is not supported here.",
    ...
    configFields: [
      {
        ...
        hint: "The Cloud project number shown on the Chat API page. Authentication Audience must be set to Project Number, and this is what every delivery's token is checked against.",
      },
      {
        key: "bot_user_name",
        label: "Bot user name (optional)",
        type: "text",
        placeholder: "users/1234567890",
        hint: "The app's own Chat user resource name, used to recognize mentions of itself.",
      },
    ],
  },
  {
    ...
    transport: "webhook",
    credentialLabel:
      "None here — the carrier credential lives on the telephony account",
    credentialOptional: true,
    editOnly: true,
    whereToGetIt:
      "An SMS account is created automatically for a telephony account of the same id; configure the carrier and its credential under Telephony.",
    ...
    configFields: [
      {
        ...
        type: "text",
        hint: "The carrier's signing key for inbound webhook verification, when the carrier publishes one.",
      },
      {
        key: "session_scope",
        label: "Session scope (optional)",
        type: "text",
        placeholder: "conversation",
        hint: "Which durable session a text thread maps onto.",
      },
    ],
  },
  {
    ...
    kind: "extension",
    label: "Executable extension",
    transport: "long_poll",
    credentialLabel:
      "None here — the extension holds its own credentials in its declared secret slots",
    credentialOptional: true,
    whereToGetIt:
      "Install a channel extension under Settings > Extensions, grant it the origins it asks for, fill in its secret slots, then name it here.",
    docsUrl:
      "https://github.com/sarollahi/little-monkey/blob/develop/docs/executable-extensions.md",
    configFields: [
      {
        key: "extension_id",
        label: "Extension",
        type: "text",
        required: true,
        placeholder: "dev.example.chat",
        hint: "The installed extension that speaks for this account. Both this and the capability are required: naming only the capability would resolve to whichever extension declares that id today.",
      },
      {
        key: "capability_id",
        ...
        required: true,
        placeholder: "room",
        hint: "Which of that extension's declared channel capabilities this account uses.",
      },
    ],
  },
];
...
export const UNIVERSAL_CONFIG_FIELDS: ProviderConfigField[] = [
  {
    key: "max_attachment_bytes",
    label: "Max attachment size (bytes)",
    type: "number",
    placeholder: "16777216",
    hint: "Per file, inbound and outbound. The application ceiling still applies.",
  },
  {
    key: "max_attachment_excerpt_chars",
    label: "Max text excerpt (characters)",
    type: "number",
    placeholder: "4000",
    hint: "How much of an inbound text file is quoted into the conversation.",
  },
  {
    key: "max_listed_attachments",
    label: "Max attachments per message",
    type: "number",
    placeholder: "8",
    hint: "How many files one message may list or carry.",
  },
];
...
export function missingRequiredConfig(
  ...
  return fields
    .filter(
      (field) =>
        field.required && (values[field.key] ?? "").trim().length === 0,
    )
    .map((field) => field.label);
}

export const channelsList = () =>
  invoke<{ accounts: ChannelAccount[] }>("channels_list");
export const channelsAdd = (
  kind: string,
  label: string,
  config: string | null,
...
export const channelsUpdateRoute = (
  routeId: string,
  recipe: string,
  options: RouteOptions,
) =>
  invoke<{ route: ChannelRoute }>("channels_update_route", {
    routeId,
    recipe,
    options,
  });
...
export const channelsRemoveRoute = (routeId: string) =>
  invoke<void>("channels_remove_route", { routeId });
export const channelsEvents = (accountId: string, limit = 20) =>
  invoke<{ events: ChannelEvent[] }>("channels_events", { accountId, limit });
export const channelsRemove = (accountId: string) =>
  invoke<void>("channels_remove", { accountId });
...
export function needsPublicCallback(kind: string): boolean {
  return (
    kind !== "sms" &&
    PROVIDER_GUIDES.find((guide) => guide.kind === kind)?.transport ===
      "webhook"
  );
}
