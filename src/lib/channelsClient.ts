...
export const PROVIDER_GUIDES: ProviderGuide[] = [
  ...
  { kind: "google_chat", label: "Google Chat", transport: "webhook", credentialLabel: "Service account key (paste the whole JSON file)", whereToGetIt: "Google Cloud Console → Chat API → create a service account and download its key. Paste the file's contents unchanged. On the same API's Configuration page, set Connection settings to HTTP endpoint URL, paste the callback URL below, and set Authentication Audience to match the setting you pick here — Google signs its deliveries differently for each, and the one that does not match is refused.", docsUrl: "https://developers.google.com/chat/api/guides/auth", configFields: [{ key: "auth_audience", label: "Authentication Audience", type: "text", placeholder: "project_number", hint: "Type project_number if Google Chat's Authentication Audience is set to Project Number (the default), or app_url if it is set to App URL." }, { key: "project_number", label: "Project number", type: "text", hint: "Required when Authentication Audience is Project Number: it is what the delivery's token is checked against." }, { key: "bot_user_name", label: "Bot user name (optional)", type: "text", placeholder: "users/1234567890", hint: "The app's own Chat user resource name, used to recognize mentions of itself." }] },
  {
