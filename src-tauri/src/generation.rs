...
/// metadata" here and nothing but a non-zero exit code anywhere else. Wide
/// enough that a native abort's `libc++abi` line survives the backtrace printed
/// after it; `engine_failure_detail` reduces it to what a person reads.
...
pub async fn poll_job(
    ...
    loop {
        let response =
            match crate::egress::send(client.get(format!("{base_url}/sdcpp/v1/jobs/{job_id}")))
                .await
            {
        ...
        let body: Value = match response.json().await {
            ...
            Err(error) => {
                let Some(delay) = retries.next() else {
                    return Err(format!(
                        "Generation engine returned an unreadable job: {error}"
                    ));
                };
                tokio::time::sleep(*delay).await;
                continue;
...
mod tests {
    ...
    fn qwen_image_2_1_int8_convrot_defaults_to_cpu_offload() {
        ...
        explicit.extra_launch_args = vec!["--params-backend".to_string(), "*=metal".to_string()];
