...
fn reject_unexpected_install_entries(
        ...
        .map_err(|source| io_at("list installed MLX package", directory, source))?
    {
        ...
        if metadata.is_dir() {
            ...
            && !is_generated_bytecode_cache(&relative_text, expected_paths)
        {
            return Err(invalid(
                "install",
...
/// Python may materialize a tagged `.pyc` beside an imported signed module.
/// It is a regenerable cache, not package payload, so it must not invalidate a
/// verified install after the first MLX video request.
fn is_generated_bytecode_cache(relative: &str, expected_paths: &BTreeSet<String>) -> bool {
    let Some((directory, filename)) = relative.rsplit_once("/__pycache__/") else {
        return false;
    };
    ...
    let source = if directory.is_empty() {
        format!("{source_stem}.py")
    } else {
        format!("{directory}/{source_stem}.py")
    };
    expected_paths.contains(&source)
}
...
pub(crate) mod tests {
    ...
    fn installer_rejects_tamper_and_atomically_verifies_active_runtime() {
        ...
        let cache = installed
            .version_directory
            .join("service/__pycache__/mlx_server.cpython-314.pyc");
        fs::create_dir_all(cache.parent().unwrap()).expect("create bytecode cache");
        fs::write(&cache, b"regenerable cache").expect("write bytecode cache");
        installer
            .verify_active()
            .expect("generated bytecode cache is ignored");

        let unexpected_cache = installed
            .version_directory
            .join("service/__pycache__/unexpected.cpython-314.pyc");
        fs::write(&unexpected_cache, b"unsigned cache").expect("write unexpected cache");
        assert!(matches!(
            installer.verify_active(),
            Err(MlxError::Invalid { .. })
        ));
        fs::remove_file(&unexpected_cache).expect("remove unexpected cache");
