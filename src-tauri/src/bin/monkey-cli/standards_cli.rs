...
fn load_document(workspace: &Path) -> Result<Value, String> {
    ...
    if !path.exists() {
        return Ok(
            json!({"schema_version":1,"workspace_id":workspace.display().to_string(),"generated_at_ms":now_ms(),"standards":[]}),
        );
    }
    let bytes =
...
fn validate_document(value: &Value) -> Result<(), String> {
    let object = value
        .as_object()
        .ok_or_else(|| "standards document must be a JSON object".to_string())?;
    if object.get("schema_version").and_then(Value::as_u64) != Some(1) {
        return Err("unsupported standards schema version".to_string());
    }
    if !object.get("workspace_id").is_some_and(Value::is_string) {
        return Err("standards document is missing workspace_id".to_string());
    }
    let standards = object
        .get("standards")
        .and_then(Value::as_array)
        .ok_or_else(|| "standards document is missing standards[]".to_string())?;
    for standard in standards {
        let object = standard
            .as_object()
            .ok_or_else(|| "standard entry must be an object".to_string())?;
        let id = object
            .get("standard_id")
            .and_then(Value::as_str)
            .ok_or_else(|| "standard is missing standard_id".to_string())?;
        if !object.get("title").is_some_and(Value::is_string)
            || !object.get("body").is_some_and(Value::is_string)
        {
            return Err(format!("standard {id} is missing title/body"));
        }
        let digest = object
            .get("content_sha256")
            .and_then(Value::as_str)
        ...
        if digest.len() != 64 || !digest.bytes().all(|byte| byte.is_ascii_hexdigit()) {
            return Err(format!("standard {id} has invalid content_sha256"));
        }
        if !object.get("evidence").is_some_and(Value::is_array) {
            return Err(format!("standard {id} has malformed evidence"));
        }
    }
    Ok(())
}
...
fn standards(value: &Value) -> Result<&Vec<Value>, String> {
    value
        .get("standards")
        .and_then(Value::as_array)
        .ok_or_else(|| "standards[] missing".to_string())
}
...
fn evidence(workspace: &Path, relative: &str, kind: &str, supports: bool) -> Option<Value> {
    ...
    let excerpt = text
        ...
        .collect::<String>();
    Some(
        json!({"path":relative.replace('\\', "/"),"line":if excerpt.is_empty(){Value::Null}else{json!(1)},"excerpt":excerpt,"sha256":digest_bytes(&bytes),"kind":kind,"supports":supports}),
    )
}
...
fn scan_files(workspace: &Path) -> Vec<String> {
    ...
    {
        if result.len() >= MAX_SCAN_FILES || bytes >= MAX_SCAN_BYTES {
            break;
        }
        ...
        let len = entry
            ...
            .unwrap_or(MAX_EVIDENCE_BYTES + 1);
...
fn file_contains(workspace: &Path, relative: &str, needles: &[&str]) -> bool {
    let bytes = match read_bounded(&workspace.join(relative)) {
        Some(bytes) => bytes,
        None => return false,
    };
    let text = String::from_utf8_lossy(&bytes);
...
fn recurring_candidate(
    ...
    let paths = source
        .iter()
        .filter(|path| file_contains(workspace, path, needles))
        .take(8)
        .cloned()
        .collect::<Vec<_>>();
    if paths.len() < 3 {
    ...
    let evidence = paths
        .iter()
        .filter_map(|path| evidence(workspace, path, "code", true))
        .collect();
    Some(candidate(
        id,
        title,
        body,
        evidence,
        (0.7 + paths.len() as f64 * 0.03).min(0.95),
        tags,
        &[],
        &[],
        &[],
...
fn discover_repository(workspace: &Path) -> Vec<Value> {
    ...
    let source = files
        .iter()
        .filter(|path| source_extension(path))
        .cloned()
        .collect::<Vec<_>>();
    ...
    if tests.len() >= 3 {
        ...
        for path in &tests {
            let style = if path.contains("/__tests__/") {
                "__tests__ directory"
        ...
        if let Some((style, supporting)) = styles.iter().max_by_key(|(_, paths)| paths.len()) {
            let mut items = supporting
                .iter()
                .take(5)
                .filter_map(|path| evidence(workspace, path, "test", true))
                .collect::<Vec<_>>();
            for path in styles
                .iter()
                .filter(|(other, _)| other != &style)
                .flat_map(|(_, paths)| paths)
                .take(5)
            {
                if let Some(item) = evidence(workspace, path, "test", false) {
                    items.push(item);
                }
            }
    ...
    if let Some((root, supporting)) = roots.iter().max_by_key(|(_, paths)| paths.len()) {
        if supporting.len() >= 3 {
            ...
            for path in roots
                ...
                .take(4)
            {
                if let Some(item) = evidence(workspace, path, "code", false) {
                    items.push(item);
                }
            }
    ...
    for layer in [
        ...
        let matching = source
            .iter()
            .filter(|path| path.split('/').any(|part| part == layer))
            .take(8)
            .cloned()
            .collect::<Vec<_>>();
        if matching.len() >= 3 {
    ...
    if !git_docs.is_empty() {
        let items = git_docs
            .iter()
            .filter_map(|path| evidence(workspace, path, "documentation", true))
            .collect();
...
fn policy_equal(left: &Value, right: &Value) -> bool {
    ["title", "body", "applicability", "severity", "tags"]
        .iter()
...
fn merge_candidates(document: &mut Value, discovered: Vec<Value>) -> Result<(), String> {
    ...
    for mut candidate in discovered {
        let id = candidate["standard_id"]
            .as_str()
            .unwrap_or_default()
            .to_string();
        if let Some(existing) = entries
            .iter_mut()
            .find(|entry| entry["standard_id"].as_str() == Some(id.as_str()))
        {
            if policy_equal(existing, &candidate) {
                if existing["status"] != "approved" {
                    existing["evidence"] = candidate["evidence"].take();
                    existing["confidence"] = candidate["confidence"].take();
                }
                continue;
            }
            ...
            if existing["status"] == "approved" {
                ...
                let history = existing
                    ...
                    .and_then(Value::as_array_mut)
                    .ok_or_else(|| format!("standard {id} has malformed revision_history"))?;
...
fn find_standard<'a>(document: &'a Value, id: &str) -> Result<&'a Value, String> {
    standards(document)?
        .iter()
        .find(|entry| entry["standard_id"].as_str() == Some(id))
        .ok_or_else(|| format!("unknown standard id {id}"))
}
fn find_standard_mut<'a>(document: &'a mut Value, id: &str) -> Result<&'a mut Value, String> {
    standards_mut(document)?
        .iter_mut()
        .find(|entry| entry["standard_id"].as_str() == Some(id))
        .ok_or_else(|| format!("unknown standard id {id}"))
}
...
fn approve(document: &mut Value, id: &str) -> Result<(), String> {
    ...
    if !standard["pending_revision"].is_null() {
        ...
        for key in [
            ...
            standard[key] = pending[key].clone();
        }
    }
    standard["status"] = "approved".into();
    standard["approved_at_ms"] = json!(now_ms());
    standard["last_verified_at_ms"] = json!(now_ms());
    standard["drift"] = "healthy".into();
    standard["pending_revision"] = Value::Null;
    Ok(())
}
...
fn reject(document: &mut Value, id: &str) -> Result<(), String> {
    ...
    if !standard["pending_revision"].is_null() {
        ...
        standard["status"] = "rejected".into();
    }
    Ok(())
}

fn drift_report(workspace: &Path, standard: &Value) -> Value {
    let supporting = standard["evidence"]
        .as_array()
        .map(|items| {
            items
                .iter()
                .filter(|item| item["supports"].as_bool() == Some(true))
                .collect::<Vec<_>>()
        })
        .unwrap_or_default();
    let unchanged = supporting
        ...
        .filter(|item| {
            ...
            let expected = item["sha256"].as_str().unwrap_or_default();
            fs::read(workspace.join(path))
                .map(|bytes| digest_bytes(&bytes) == expected)
                .unwrap_or(false)
        })
        .count();
...
fn conflict_pairs(document: &Value) -> Result<Vec<String>, String> {
    let active = standards(document)?
        .iter()
        .filter(|entry| {
            matches!(
                entry["status"].as_str(),
                Some("approved" | "candidate" | "conflicting")
            )
        })
        .filter_map(|entry| entry["standard_id"].as_str())
        .collect::<BTreeSet<_>>();
    let mut pairs = BTreeSet::new();
    for entry in standards(document)? {
        let Some(id) = entry["standard_id"].as_str() else {
            continue;
        };
        ...
        {
            if active.contains(id) && active.contains(other) {
                let (a, b) = if id <= other {
                    (id, other)
                } else {
                    (other, id)
...
fn preview(
    ...
    for standard in standards(document)? {
        ...
        if standard["status"] != "approved"
            || matches!(standard["drift"].as_str(), Some("contradicted" | "unknown"))
            || conflicted.contains(id)
        {
            continue;
        }
        let mut score = if standard["severity"] == "required" {
            100
        } else {
            0
        };
        ...
        for field in ["tags", "task_keywords", "languages", "frameworks"] {
            let values =
                if field == "task_keywords" || field == "languages" || field == "frameworks" {
                    standard["applicability"].get(field)
                } else {
                    standard.get(field)
                };
            for value in values
                ...
                .filter_map(Value::as_str)
            {
                if query.contains(&value.to_ascii_lowercase()) {
                    score += 20;
                    reasons.push(format!("{field}:{value}"));
                }
            }
        }
        for glob in standard["applicability"]["globs"]
            .as_array()
            .into_iter()
            .flatten()
            .filter_map(Value::as_str)
        {
            let hint = glob.replace("**/", "").replace('*', "");
            if hint.len() > 1
                && file_hints.iter().any(|path| {
                    path.to_ascii_lowercase()
                        .contains(&hint.to_ascii_lowercase())
                })
            {
                score += 25;
                reasons.push(format!("files:{glob}"));
            }
        }
        if score <= 0 {
            continue;
        }
        let chars = standard["body"]
            .as_str()
            .unwrap_or_default()
            .chars()
            .count()
    ...
    ranked.sort_by(|a, b| {
        b.0.cmp(&a.0).then_with(|| {
            a.2["standard_id"]
                .as_str()
                .cmp(&b.2["standard_id"].as_str())
        })
    });
    let mut used = 0usize;
    let mut selected = Vec::new();
    let total = ranked.len();
    for (_, chars, item) in ranked {
    ...
    Ok(
        json!({"schema_version":1,"selected":selected,"omitted":total-selected.len(),"total_chars":used,"budget_chars":budget}),
    )
}
...
fn import_document(workspace: &Path, path: &Path, document: &mut Value) -> Result<(), String> {
    ...
    let incoming: Value = serde_json::from_slice(
        &fs::read(&absolute)
            .map_err(|error| format!("failed to read {}: {error}", absolute.display()))?,
    )
    .map_err(|error| format!("failed to parse {}: {error}", absolute.display()))?;
...
fn print_standard(standard: &Value, json_output: bool) -> Result<(), String> {
    if json_output {
        println!(
            "{}",
            serde_json::to_string_pretty(standard).map_err(|error| error.to_string())?
        );
    } else {
        println!(
            "{}@v{}  {}  {}",
            standard["standard_id"].as_str().unwrap_or_default(),
            standard["version"].as_u64().unwrap_or(1),
            standard["status"].as_str().unwrap_or("unknown"),
            standard["title"].as_str().unwrap_or_default()
        );
        println!("{}", standard["body"].as_str().unwrap_or_default());
        for item in standard["evidence"].as_array().into_iter().flatten() {
            println!(
                "  {} {}:{}  {}",
                if item["supports"].as_bool() == Some(false) {
                    "counterexample"
                } else {
                    "evidence"
                },
                ...
                item["line"]
                    ...
                    .map(|v| v.to_string())
                    .unwrap_or_else(|| "?".into()),
                item["excerpt"].as_str().unwrap_or_default()
            );
        }
    }
    Ok(())
...
pub fn run(action: &StandardsCmd, workspace: &Path) -> Result<(), String> {
    ...
    match action {
        StandardsCmd::Discover { json, dry_run } => {
            let discovered = discover_repository(&workspace);
            let mut document = load_document(&workspace)?;
            merge_candidates(&mut document, discovered.clone())?;
            if !dry_run {
                save_document(&workspace, &mut document)?;
            }
            if *json {
                println!(
                    "{}",
                    serde_json::to_string_pretty(&discovered).map_err(|error| error.to_string())?
                );
            } else {
                println!(
                    "Discovered {} evidence-backed standard candidates{}.",
                    discovered.len(),
                ...
                for item in discovered {
                    println!(
                        ...
                        item["confidence"].as_f64().unwrap_or(0.0) * 100.0,
        ...
        StandardsCmd::List { status, json } => {
            let document = load_document(&workspace)?;
            let filtered = standards(&document)?
                .iter()
                .filter(|entry| {
                    status
                        .map(|wanted| entry["status"].as_str() == Some(wanted.as_str()))
                        .unwrap_or(true)
                })
                .collect::<Vec<_>>();
            if *json {
                ...
                for item in filtered {
                    println!(
                        "{}@v{}\t{}\t{}\t{}",
                        item["standard_id"].as_str().unwrap_or_default(),
                        item["version"].as_u64().unwrap_or(1),
                        item["status"].as_str().unwrap_or("unknown"),
                        item["drift"].as_str().unwrap_or("unknown"),
                        item["title"].as_str().unwrap_or_default()
                    );
                }
            }
        ...
        StandardsCmd::Show { standard_id, json } => {
            let document = load_document(&workspace)?;
            print_standard(find_standard(&document, standard_id)?, *json)
        }
        StandardsCmd::Approve { standard_id, json } => {
            let mut document = load_document(&workspace)?;
            approve(&mut document, standard_id)?;
            save_document(&workspace, &mut document)?;
            print_standard(find_standard(&document, standard_id)?, *json)
        }
        StandardsCmd::Reject { standard_id, json } => {
            let mut document = load_document(&workspace)?;
            reject(&mut document, standard_id)?;
            save_document(&workspace, &mut document)?;
            print_standard(find_standard(&document, standard_id)?, *json)
        }
        StandardsCmd::Drift { json, no_write } => {
            let mut document = load_document(&workspace)?;
            let reports = standards(&document)?
                .iter()
                .map(|entry| drift_report(&workspace, entry))
                .collect::<Vec<_>>();
            if !no_write {
                let timestamp = now_ms();
                for (entry, report) in standards_mut(&mut document)?.iter_mut().zip(&reports) {
                    entry["drift"] = report["current"].clone();
                    entry["last_verified_at_ms"] = json!(timestamp);
                    if entry["status"] == "approved" && report["current"] == "contradicted" {
            ...
            if *json {
                ...
                for report in reports {
                    println!(
                        ...
                        report["standard_id"].as_str().unwrap_or_default(),
                        report["previous"].as_str().unwrap_or("unknown"),
                        report["current"].as_str().unwrap_or("unknown"),
        ...
        StandardsCmd::Conflicts { json } => {
            let document = load_document(&workspace)?;
            let pairs = conflict_pairs(&document)?;
            if *json {
                println!(
                    "{}",
                    serde_json::to_string_pretty(&json!({"conflicts":pairs}))
                        .map_err(|error| error.to_string())?
                );
            } else if pairs.is_empty() {
                println!("No unresolved standards conflicts.");
            } else {
                for pair in pairs {
                    println!("{pair}");
                }
        ...
        StandardsCmd::Preview {
            ...
            let document = load_document(&workspace)?;
            let report = preview(&document, task, files, *budget_chars)?;
            if *json {
                println!(
                    "{}",
                ...
                println!(
                    "{} chars; {} omitted",
                    report["total_chars"], report["omitted"]
                );
            }
            Ok(())
        }
        StandardsCmd::Import { path, json } => {
            let mut document = load_document(&workspace)?;
            import_document(&workspace, path, &mut document)?;
            save_document(&workspace, &mut document)?;
            if *json {
                println!(
                    "{}",
                ...
                println!("Imported standards from {}.", path.display());
            }
            Ok(())
        }
        StandardsCmd::Export { path, json } => {
            let document = load_document(&workspace)?;
            let output = export_document(&workspace, path, &document)?;
            if *json {
                println!(
                    "{}",
                ...
                println!("Exported standards to {}.", output.display());
            }
            Ok(())
        }
    }
...
mod tests {
    ...
    fn approved(id: &str, tags: &[&str], body: &str) -> Value {
        let mut value = candidate(id, id, body, vec![], 1.0, tags, &[], &[], &[], tags);
        value["status"] = "approved".into();
        value
    }

    #[test]
    fn preview_is_bounded_and_relevant() {
        let document = json!({"schema_version":1,"workspace_id":"x","generated_at_ms":1,"standards":[approved("react",&["react","component"],"A"),approved("rust",&["rust","cargo"],"B")]});
        let result = preview(
            &document,
            "Add React component",
            &[],
        ...
        .unwrap();
        assert_eq!(result["selected"].as_array().unwrap().len(), 1);
        assert_eq!(result["selected"][0]["standard_id"], "react");
    }

    #[test]
    fn merge_never_silently_replaces_approved_policy() {
        let mut current = approved("x", &["x"], "old");
        current["version"] = json!(2);
        let mut document = json!({"schema_version":1,"workspace_id":"x","generated_at_ms":1,"standards":[current]});
        let changed = candidate("x", "x", "new", vec![], 1.0, &["x"], &[], &[], &[], &["x"]);
        merge_candidates(&mut document, vec![changed]).unwrap();
        assert_eq!(document["standards"][0]["body"], "old");
        assert_eq!(document["standards"][0]["pending_revision"]["body"], "new");
    }

    #[test]
    fn repository_text_cannot_auto_approve() {
        let temp = std::env::temp_dir().join(format!("lm-standards-{}", now_ms()));
        fs::create_dir_all(temp.join("src/security")).unwrap();
        for name in ["a.ts", "b.ts", "c.ts"] {
            fs::write(
                temp.join("src/security").join(name),
                "permission validate policy",
            )
            .unwrap();
        }
        let discovered = discover_repository(&temp);
        assert!(discovered
            .iter()
