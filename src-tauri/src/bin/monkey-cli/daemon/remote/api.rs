...
impl RemoteApi {
    ...
    #[cfg(test)]
    pub(crate) fn with_desktop(mut self, desktop: Arc<DesktopControlRuntime>) -> Self {
    ...
    fn talk_ticket(
        ...
        let (
            ...
            match (
                ...
                (Some(route_id), Some(generation), Some(role)) => {
                    ...
                    match role {
                        "input" => {
                            if route.input_endpoint != expected_input {
                                return Err((
                                    403,
                                    "This paired device is not the selected VoiceRoute microphone"
                                        .to_string(),
                                ));
                            }
                            if !effective.contains(&DeviceCapability::VoiceStream) {
                                return Err((
                                    403,
                                    "This device's voice_stream capability is not effective"
                                        .to_string(),
                                ));
                            }
                        }
                        "output" => {
                            if route.output_endpoint != expected_output
                                || route.input_endpoint == expected_input
                            {
                                return Err((
                                    403,
                                    "This paired device is not the independent VoiceRoute speaker"
                                        .to_string(),
                                ));
                            }
                            if !effective.contains(&DeviceCapability::AudioPlayback) {
                                return Err((
                                    403,
                                    "This device's audio_playback capability is not effective"
                                        .to_string(),
                                ));
                            }
                        }
                    ...
                    let cursor = store
                        .latest_voice_route_event_id(&route.session_id)
                        .map_err(internal)?;
                    (
                        route.session_id,
                        Some(route.route_id),
        ...
        let issued = TalkTicketResponse::issue(session_id, now_ms, DEFAULT_TALK_TICKET_TTL_MS)
        ...
        let mut route_metrics = None;
        {
            ...
            if let (Some(route_id), Some(generation), Some(role)) = (
                ...
                route_metrics = Some(super::talk::TalkRouteReport {
                    ...
                    output_kind: super::talk::TalkRouteReport::endpoint_kind(
    ...
    pub(crate) fn talk_output_route_live(&self, authorization: &TalkSocketAuthorization) -> bool {
        let (Some(route_id), Some(generation)) = (
            authorization.route_id.as_deref(),
        ...
        let Some(device) = store.device(&authorization.device_id).ok().flatten() else {
            return false;
        };
        if !device.active() {
            return false;
        ...
        let surface = store
            .device_surface(&authorization.device_id)
            .ok()
            .flatten();
        let effective = effective_capabilities(&device.capabilities, surface.as_ref());
        let Ok(Some(route)) = store.voice_route_by_id(route_id) else {
            return false;
        };
        route.state == "active"
            && route.generation == generation
    ...
    pub(crate) fn talk_route_command_id(
        &self,
        ...
        let store = self.store.lock().ok()?;
        let route = store.voice_route_by_id(route_id).ok().flatten()?;
        if route.generation != authorization.route_generation?
            || route.session_id != authorization.session_id
        {
            return None;
        }
    ...
    fn realtime_host_route(
        &self,
        session_id: &str,
        generation: u64,
    ) -> Result<super::store::VoiceRouteRecord, String> {
        let store = self
            .store
            .lock()
            .map_err(|_| "Remote state lock was poisoned".to_string())?;
    ...
    pub(crate) fn push_realtime_output_from_host(
        ...
        super::realtime_bridge::validate_pcm(&bytes)?;
        self.realtime_media
            .push_output(session_id, generation, bytes)
    }
    ...
    pub(crate) fn take_realtime_output_for_device(
        ...
        self.realtime_media
            .pop_output(&authorization.session_id, generation)
    }
...
impl super::talk::TalkTurns for TalkSessionTurns {
    fn submit(&self, session_id: &str, client_key: &str, text: &str) -> Result<String, String> {
        if let (Some(_route_id), Some(generation)) =
            (self.route_id.as_deref(), self.route_generation)
        {
                ...
                match event.kind.as_str() {
                    ...
                    "host_interrupt" => {
                        ...
                        if progress.delta.is_empty() {
                            progress.error =
                                Some("This turn was interrupted on the host.".to_string());
                        }
                    }
                    _ => {}
                }
    ...
    fn still_granted(&self, device_id: &str) -> bool {
        ...
        let (Some(route_id), Some(generation)) = (self.route_id.as_deref(), self.route_generation)
        else {
            return true;
        };
        let Ok(store) = self.api.store.lock() else {
            return false;
        };
        let Ok(Some(route)) = store.voice_route_by_id(route_id) else {
            return false;
        ...
        if self.route_output_to_socket {
            let Ok(Some(device)) = store.device(&self.device_id) else {
                return false;
            };
...
mod tests {
    ...
    use little_monkey_lib::desktop_control::ControlAction;
    ...
    #[cfg(target_os = "windows")]
    struct RemoteWindowsE2EConsent;

    #[cfg(target_os = "windows")]
    impl crate::daemon::remote::desktop::ConsentPrompter for RemoteWindowsE2EConsent {
        fn confirm_session(
            &self,
            _device_label: &str,
        ) -> crate::daemon::remote::desktop::SessionConsent {
            crate::daemon::remote::desktop::SessionConsent::AllowPerAction
        }
    ...
    #[cfg(target_os = "windows")]
    #[test]
    #[ignore = "requires an interactive Windows desktop, the native fixture, and signed remote calls"]
    fn signed_remote_windows_desktop_control_is_a_real_runner_golden_flow() {
        assert_eq!(
        ...
        let result = (|| -> Result<(), String> {
            ...
            let state = Arc::new(
                little_monkey_lib::desktop_control::DesktopControlState::production_with_lock(
                    root.join("desktop_control.lock"),
                ),
            );
            let runtime = DesktopControlRuntime::for_test(
                state.clone(),
                Arc::new(RemoteWindowsE2EConsent),
                paths,
            );
            let api = api.with_desktop(runtime.clone());
            let mut sequence = 1;
            let mut call = |command: &str, method: &str, path: &str, body: Vec<u8>| {
                let request = signed(&device, &secret, sequence, command, method, path, &body);
                sequence += 1;
                api.handle(request, 2_000)
            };
            let json = |response: ApiResponse| -> Result<serde_json::Value, String> {
                if response.status >= 300 {
                    return Err(format!(
                        "signed remote request failed with {}: {}",
                        response.status,
            ...
            let start_body = serde_json::to_vec(&DesktopControlStartRequest {
                allowlist: vec![format!("process:{}", child.id())],
                batch_mode: false,
                allowed_windows: Vec::new(),
                lifetime_ms: Some(900_000),
                allow_screenshots: Some(true),
                allow_keyboard_input: Some(true),
                allow_clipboard_read: Some(false),
                approval_policy: Some(
                    little_monkey_lib::desktop_control::ApprovalPolicy::PerAction,
                ),
            })
            .map_err(|error| error.to_string())?;
            let discovered = json(call(
                "remote-start-discovery",
                "POST",
                "/v1/remote/desktop-control/start",
                start_body.clone(),
            ))?;
            let discovery_session = discovered["session"]["sessionId"]
                .as_str()
                .ok_or_else(|| "signed remote start returned no session id".to_string())?
                .to_string();
            let list_body = serde_json::to_vec(&DesktopControlTargetRequest {
                session_id: discovery_session.clone(),
            ...
            .map_err(|error| error.to_string())?;
            json(call(
                "remote-stop-discovery",
                "POST",
                "/v1/remote/desktop-control/stop",
                stop_discovery,
            ))?;

            let exact_start = serde_json::to_vec(&DesktopControlStartRequest {
                allowlist: vec![application_id.clone()],
                batch_mode: false,
                allowed_windows: vec![window_id.clone()],
                lifetime_ms: Some(900_000),
                allow_screenshots: Some(true),
                allow_keyboard_input: Some(true),
                allow_clipboard_read: Some(false),
            ...
            let session_id = started["session"]["sessionId"]
                .as_str()
                .ok_or_else(|| "signed remote exact start returned no session id".to_string())?
                .to_string();
            let target_body = |session_id: &str| {
                serde_json::to_vec(&DesktopControlTargetRequest {
                    session_id: session_id.to_string(),
            ...
            let inspection = json(call(
                "remote-inspect",
                "POST",
                "/v1/remote/desktop-control/inspect",
                target_body(&session_id),
            ))?;
            let elements = inspection["inspection"]["elements"]
                .as_array()
                .ok_or_else(|| "signed remote inspection returned no elements".to_string())?;
            ...
            let dark_id = element_id("Dark mode", "click")
                .ok_or_else(|| "signed remote inspection did not expose Dark mode".to_string())?;
            let profile_id = element_id("Profile name", "set_value").ok_or_else(|| {
                "signed remote inspection did not expose Profile name".to_string()
            })?;
            let save_id = element_id("Save profile", "click").ok_or_else(|| {
                "signed remote inspection did not expose Save profile".to_string()
            })?;
            let action_body = |action: ControlAction| {
                serde_json::to_vec(&DesktopControlActionRequest {
            ...
            let mut action =
                |command: &str, value: ControlAction| -> Result<serde_json::Value, String> {
                    json(call(
                        command,
                        "POST",
                        "/v1/remote/desktop-control/action",
                        action_body(value),
                    ))
                };
            action("remote-focus", ControlAction::Focus)?;
            action(
                "remote-toggle",
            ...
            let screenshot = json(call(
                "remote-screenshot",
                "POST",
                "/v1/remote/desktop-control/screenshot",
                target_body(&session_id),
            ))?;
            if screenshot["artifact_id"].as_str().is_none() {
                return Err("signed remote screenshot returned no artifact id".to_string());
            }
            let pause_body = serde_json::to_vec(&DesktopControlStopRequest {
                session_id: session_id.clone(),
            })
            .map_err(|error| error.to_string())?;
            json(call(
                "remote-pause",
                "POST",
                "/v1/remote/desktop-control/pause",
                pause_body.clone(),
            ))?;
            let paused_action = call(
                "remote-action-while-paused",
                "POST",
                "/v1/remote/desktop-control/action",
                serde_json::to_vec(&DesktopControlActionRequest {
                    session_id: session_id.clone(),
                    target_application_id: application_id.clone(),
                    target_window_id: Some(window_id.clone()),
                    action: ControlAction::Focus,
                })
                .map_err(|error| error.to_string())?,
            );
            if paused_action.status != 409 {
                return Err(format!(
                    "paused remote action returned {}",
                    paused_action.status
                ));
            }
            json(call(
                "remote-resume",
                "POST",
                "/v1/remote/desktop-control/resume",
                pause_body,
            ))?;
            json(call(
                "remote-stop-exact",
                "POST",
                "/v1/remote/desktop-control/stop",
                serde_json::to_vec(&DesktopControlStopRequest { session_id })
                    .map_err(|error| error.to_string())?,
            ))?;
            ...
            if audit_text.contains("hello") {
                return Err("signed remote audit contained a value-writing payload".to_string());
            }
            Ok(())
        })();
        ...
        let _ = child.kill();
        let _ = child.wait();
        result.expect("signed remote Windows golden flow must pass");
    }
    ...
    fn route_events(
        api: &RemoteApi,
        session_id: &str,
    ) -> Vec<super::super::store::VoiceRouteEventRecord> {
    ...
    async fn a_routed_paired_microphone_lands_its_turn_in_the_host_chosen_conversation() {
        ...
        let (mut socket, generation, session_id) = open_route_socket(
            &api, &device_id, &secret, 2, &route, "input", address, now_ms,
        )
        .await;
        let _ = read_until(&mut socket, "ready").await;
    ...
    async fn a_routed_paired_speaker_gets_the_next_chunk_only_after_acknowledging_the_last() {
        ...
        let address = spawn_talk_server(api.clone()).await;
        let (mut socket, generation, session_id) = open_route_socket(
            &api, &device_id, &secret, 2, &route, "output", address, now_ms,
        )
        .await;
        let _ = read_until(&mut socket, "ready").await;
        socket
        ...
        assert_eq!(
            played.len(),
            1,
            "one completed response, one completion row"
        );
    ...
    async fn a_duplex_paired_device_hears_the_answer_on_the_socket_it_spoke_into() {
        ...
        let address = spawn_talk_server(api.clone()).await;
        let (mut socket, generation, session_id) = open_route_socket(
            &api, &device_id, &secret, 2, &route, "duplex", address, now_ms,
        )
        .await;
        let _ = read_until(&mut socket, "ready").await;
    ...
    async fn a_stale_route_generation_is_refused_at_the_handshake_and_closed_mid_socket() {
        ...
        let (mut socket, generation, session_id) = open_route_socket(
            &api, &device_id, &secret, 3, &moved, "input", address, now_ms,
        )
        .await;
        let _ = read_until(&mut socket, "ready").await;
        socket
    ...
    async fn host_waits_for_input(
        ...
        for _ in 0..200 {
            ...
            assert_eq!(
                status, 204,
                "the host ingress is open and empty, not refused"
            );
    ...
    async fn realtime_microphone_pcm_reaches_the_host_without_passing_through_transcription() {
        ...
        assert_eq!(
            sequence,
            Some(1),
            "ingress sequences start at one and are stamped"
        );
        assert_eq!(
            host_takes_input(&bridge, &route.session_id, route.generation)
                .await
                .0,
            204,
        ...
        let kinds = route_events(&api, &route.session_id)
            ...
            .collect::<Vec<_>>();
        assert!(
            kinds.contains(&"input_ready".to_string()),
            "the route came up: {kinds:?}"
        );
        assert!(
    ...
    async fn host_realtime_output_is_streamed_to_the_paired_speaker_at_the_pace_it_acknowledges() {
        ...
        assert_eq!(
            ninth["audio_sequence"], 9,
            "one acknowledgement releases one chunk"
        );
        assert_eq!(
            STANDARD
                .decode(ninth["audio_base64"].as_str().unwrap())
                .unwrap(),
            pushed[8]
        );
        assert!(
    ...
    async fn a_stale_realtime_generation_is_refused_in_both_directions() {
        ...
        socket
            ...
            .unwrap();
        assert_eq!(
            host_waits_for_input(&bridge, &route.session_id, route.generation)
                .await
                .0,
            heard
        );
        ...
        assert_eq!(
            moved.engine, "realtime",
            "a handoff does not change the engine"
        );
        ...
        assert_eq!(
            host_takes_input(&bridge, &moved.session_id, moved.generation)
                .await
                .0,
            204,
    ...
    async fn switching_between_pipeline_and_realtime_keeps_one_conversation_and_one_session() {
        ...
        socket
            ...
            .unwrap();
        assert_eq!(
            host_waits_for_input(&bridge, conversation, realtime.generation)
                .await
                .0,
            spoken
        );
        assert_eq!(
            host_takes_input(&bridge, conversation, pipeline.generation)
                .await
                .0,
            409,
        ...
        assert_eq!(
            host_takes_input(&bridge, conversation, realtime.generation)
                .await
                .0,
            409,
    ...
    async fn a_finished_routed_session_records_the_routes_shape_and_timing_and_no_audio() {
        ...
        assert_eq!(
            measured["input"]["endpoint"],
            format!("paired:{device_id}:input")
        );
        assert_eq!(measured["output"]["kind"], "paired");
        assert_eq!(
            measured["output"]["endpoint"],
            format!("paired:{device_id}:output")
        );

        let setup_ms = measured["setupMs"]
            .as_u64()
            .expect("setup latency is a number");
        assert!(
        ...
        assert!(
            !recorded_detail_bytes(&paths)
                .windows(spoken.len())
                .any(|window| window == spoken),
