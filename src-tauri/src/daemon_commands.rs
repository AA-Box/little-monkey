...
pub async fn autonomous_task_place_node(
    ...
    match kind.as_str() {
            ...
            let placement_result = (|| {
                if runner_result.get("ok").and_then(Value::as_bool) != Some(false) {
                    ...
                    let result_id = spec
                        ...
                        .as_ref()
                        .map(|base| {
                            let result = crate::execution_target::workspace_result_from_workspace(
                                base,
                                &container_workspace,
                            )
                            .map_err(|error| error.to_string())?;
                            crate::execution_target::persist_workspace_result(&data_dir, &result)
                                .map_err(|error| error.to_string())
                        })
                        .transpose()?;
                    let mut response = autonomous_placement_result(
                        &data_dir,
                        Path::new(&host_workspace),
                        &before_revision,
                        &local_snapshot.id,
        ...
        "ssh_runner" => {
            ...
            Ok(serde_json::json!({
                "ok": true,
                "summary": "SSH runner completed; review the persisted workspace result before applying it",
                "resultId": result_id,
