//! Real WhatsApp user -> Meta webhook -> installed resident daemon -> agent
//! -> Meta Cloud API -> signed delivered/read receipt acceptance.
//!
//! This example is intentionally interactive at the one boundary Meta does not
//! expose as a consumer API: an independent WhatsApp user sends the unique
//! marker from a real handset/account. Everything after that is automatic and
//! asserted. The daemon owns the webhook listener and channel runtime; this
//! process never calls the WhatsApp adapter directly and never injects a reply.
//!
//! By default the WABA callback must already point at the callback URL printed
//! by `monkey channels callback-url`. For a disposable test WABA, set
//! `WA_E2E_MUTATE_WABA_SUBSCRIPTION=1` and this harness will subscribe/override
//! the WABA through Meta's official `subscribed_apps` API, verify the override,
//! then unsubscribe during cleanup.
...
const REQUIRE_ENV: &str = "LITTLE_MONKEY_REQUIRE_WHATSAPP_INSTALLED_SERVICE_E2E";
const RECIPE: &str = "whatsapp-installed-service-e2e";
const REPLY_PREFIX: &str = "little-monkey whatsapp installed-service reply";
...
const DELIVERY_WAIT: Duration = Duration::from_secs(300);
...
fn add_account(profile: &str, phone_number_id: &str) -> Result<String, String> {
    let label = format!("whatsapp-e2e-{}", unique());
    let config = serde_json::json!({ "phone_number_id": phone_number_id }).to_string();
...
fn write_recipe(profile: &str, workspace: &Path, model_base: &str) -> Result<(), String> {
    ...
    let recipe = serde_json::json!({
        "version": 1,
        "name": RECIPE,
        "target": { "local_url": model_base, "model": "whatsapp-e2e-fixture" },
...
fn open_state_read_only(path: &Path) -> Result<Connection, String> {
...
fn find_inbound_proof(
    ...
    let connection = open_state_read_only(db)?;
    ...
    connection
        ...
        .optional()
        .map_err(|error| format!("read exact durable WhatsApp inbound: {error}"))
}

fn find_outbound_provider_id(
    db: &Path,
    account_id: &str,
    conversation_id: &str,
    expected_reply: &str,
    ...
    connection
        .query_row(
            "SELECT provider_event_id
             ...
             WHERE account_id = ?1
               ...
               AND provider_event_id NOT LIKE 'status:%'
        ...
        .optional()
        .map_err(|error| format!("read exact durable WhatsApp outbound: {error}"))
}
...
fn delivered_receipt(
    ...
    let connection = open_state_read_only(db)?;
    let delivered = format!("status:{provider_message_id}:delivered");
    let read = format!("status:{provider_message_id}:read");
    connection
        .query_row(
            "SELECT provider_event_id
             FROM channel_events
             WHERE account_id = ?1
               AND direction = 'outbound'
               AND provider_event_id IN (?2, ?3)
               AND disposition = 'accepted'
             ORDER BY received_at_ms DESC
             LIMIT 1",
            (account_id, delivered, read),
            |row| row.get(0),
        )
        .optional()
        .map_err(|error| format!("read WhatsApp delivered/read receipt: {error}"))
}
...
async fn meta_subscriptions(
    client: &reqwest::Client,
    access_token: &str,
    waba_id: &str,
) -> Result<serde_json::Value, String> {
    let response = client
        .get(format!("{GRAPH_API_BASE}/{waba_id}/subscribed_apps"))
        .bearer_auth(access_token)
        .send()
        .await
        .map_err(|error| format!("read WABA subscribed_apps: {error}"))?;
    let status = response.status();
    let payload: serde_json::Value = response
        .json()
        .await
        .map_err(|error| format!("WABA subscribed_apps JSON: {error}"))?;
...
fn subscription_has_override(payload: &serde_json::Value, callback_url: &str) -> bool {
    payload
        .get("data")
        .and_then(serde_json::Value::as_array)
        .is_some_and(|rows| {
            rows.iter().any(|row| {
                row.get("override_callback_uri")
                    .and_then(serde_json::Value::as_str)
                    == Some(callback_url)
            })
        })
...
async fn configure_disposable_waba_override(
    client: &reqwest::Client,
    access_token: &str,
    waba_id: &str,
    callback_url: &str,
    ...
    let baseline = client
        .post(format!("{GRAPH_API_BASE}/{waba_id}/subscribed_apps"))
        .bearer_auth(access_token)
        .send()
        .await
        .map_err(|error| format!("subscribe disposable WABA: {error}"))?;
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|error| format!("set disposable WABA callback override: {error}"))?;
    let status = response.status();
...
async fn unsubscribe_disposable_waba(client: &reqwest::Client, access_token: &str, waba_id: &str) {
    let _ = client
        .delete(format!("{GRAPH_API_BASE}/{waba_id}/subscribed_apps"))
        .bearer_auth(access_token)
        .send()
        .await;
}
...
fn cleanup_local(profile: Option<&str>, account_id: Option<&str>) {
    if let Some(profile) = profile {
...
struct LiveConfig {
    ...
    business_display_number: String,
    webhook_port: u16,
    mutate_waba: bool,
}
...
impl LiveConfig {
    fn from_env() -> Result<Self, String> {
        ...
        let webhook_port = std::env::var("WA_E2E_WEBHOOK_PORT")
            .unwrap_or_else(|_| "38443".to_string())
            .parse::<u16>()
            .map_err(|_| "WA_E2E_WEBHOOK_PORT must be a non-zero u16".to_string())?;
        if webhook_port == 0 {
            return Err("WA_E2E_WEBHOOK_PORT must be non-zero".to_string());
        }
        let public_base = required("WA_E2E_PUBLIC_BASE")?
            .trim_end_matches('/')
            .to_string();
        if !public_base.starts_with("https://") {
            return Err("WA_E2E_PUBLIC_BASE must be a public https:// origin".to_string());
        }
        Ok(Self {
            access_token: required("WA_E2E_ACCESS_TOKEN")?,
            app_secret: required("WA_E2E_APP_SECRET")?,
            verify_token: required("WA_E2E_VERIFY_TOKEN")?,
            phone_number_id: required("WA_E2E_PHONE_NUMBER_ID")?,
            waba_id: required("WA_E2E_WABA_ID")?,
            public_base,
            external_wa_id: required("WA_E2E_EXTERNAL_WA_ID")?,
            business_display_number: required("WA_E2E_BUSINESS_DISPLAY_NUMBER")?,
            webhook_port,
            mutate_waba: std::env::var("WA_E2E_MUTATE_WABA_SUBSCRIPTION").as_deref() == Ok("1"),
        })
    }
}
...
async fn run_case(config: &LiveConfig) -> Result<(), String> {
    if std::env::var(REQUIRE_ENV).as_deref() != Ok("1") {
        return Err(format!(
            "refusing to install a real OS service and use a live WhatsApp account unless {REQUIRE_ENV}=1"
        ));
    }

    let stamp = unique();
    let marker = format!("lm-whatsapp-installed-{stamp:x}");
    let expected_reply = format!("{REPLY_PREFIX} {marker}");
    let model = ModelFixture::spawn(marker.clone())?;
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(30))
        .build()
        .map_err(|error| format!("build Meta client: {error}"))?;
    let mut profile: Option<String> = None;
    let mut account_id: Option<String> = None;
    let mut mutated_subscription = false;

    let result: Result<(), String> = async {
        let created = create_profile()?;
        profile = Some(created.clone());
        let workspace = std::env::temp_dir().join(format!("lm-wa-e2e-workspace-{stamp:x}"));
        std::fs::create_dir_all(&workspace)
            .map_err(|error| format!("create workspace {}: {error}", workspace.display()))?;
        write_recipe(&created, &workspace, &model.base)?;

        let account = add_account(&created, &config.phone_number_id)?;
        ...
        if callback_url != expected_callback {
            return Err(format!(
                "callback mismatch: CLI advertised {callback_url:?}, expected {expected_callback:?}"
            ));
        }

        if config.mutate_waba {
            mutated_subscription = true;
            configure_disposable_waba_override(
                &client,
                &config.access_token,
                &config.waba_id,
                &callback_url,
        ...
        let subscriptions = meta_subscriptions(&client, &config.access_token, &config.waba_id).await?;
        if !subscription_has_override(&subscriptions, &callback_url) {
            return Err(format!(
                "WABA is not using the installed daemon callback {callback_url:?}: {subscriptions}. Preconfigure that override, or use a disposable WABA with WA_E2E_MUTATE_WABA_SUBSCRIPTION=1."
            ));
        }

        eprintln!(
            "\nWhatsApp installed-service acceptance is ready.\nFrom the independent WhatsApp identity {} send this exact text to {}:\n\n    {}\n\nThe harness will accept only that sender and marker.\n",
            config.external_wa_id, config.business_display_number, marker
        );
        ...
        let inbound = loop {
            if let Some(proof) =
                find_inbound_proof(&db, &account, &config.external_wa_id, &marker)?
            {
                break proof;
            }
            if Instant::now() >= inbound_deadline {
                return Err(format!(
                    "no authenticated WhatsApp message from {} containing the marker reached a durable ingress/job within {}s",
                    config.external_wa_id,
        ...
        eprintln!(
            "Meta inbound {} became ingress {} / job {}",
        ...
        let requests = model.requests();
        if !requests.iter().any(|request| request.contains(&marker)) {
            return Err("installed agent never sent the real WhatsApp marker to the model".to_string());
        }
        if !requests
            .iter()
            .any(|request| request.contains(r#""name":"send_message""#))
        ...
        let outbound_deadline = Instant::now() + Duration::from_secs(180);
        let outbound_provider_id = loop {
            if let Some(provider_id) = find_outbound_provider_id(
                &db,
                &account,
                &config.external_wa_id,
                &expected_reply,
            ...
            if Instant::now() >= outbound_deadline {
                return Err(format!(
                    "the generated reply never became a provider-named outbound event: {expected_reply:?}"
                ));
            }
            tokio::time::sleep(Duration::from_millis(500)).await;
        };
        if !outbound_provider_id.starts_with("wamid.") {
            return Err(format!(
                "WhatsApp outbound id was not a real Meta wamid: {outbound_provider_id}"
            ));
        }
        eprintln!("Meta accepted generated reply as {outbound_provider_id}");

        let receipt_deadline = Instant::now() + DELIVERY_WAIT;
        let receipt = loop {
            if let Some(receipt) = delivered_receipt(&db, &account, &outbound_provider_id)? {
                break receipt;
            }
            if Instant::now() >= receipt_deadline {
                return Err(format!(
                    "Meta never sent a signed delivered/read receipt for {outbound_provider_id} within {}s; API acceptance alone is not delivery evidence",
                    DELIVERY_WAIT.as_secs()
                ));
            }
            tokio::time::sleep(Duration::from_millis(500)).await;
        };
        eprintln!("provider-side delivery observed through signed callback: {receipt}");
        Ok(())
    }
    .await;
    ...
    if mutated_subscription {
        unsubscribe_disposable_waba(&client, &config.access_token, &config.waba_id).await;
    }
    cleanup_local(profile.as_deref(), account_id.as_deref());
    result
}
...
async fn main() {
    let config = match LiveConfig::from_env() {
        Ok(config) => config,
        Err(error) => {
            eprintln!("WhatsApp installed-service E2E configuration error: {error}");
            std::process::exit(2);
        }
    };
    if let Err(error) = run_case(&config).await {
        eprintln!("WhatsApp installed-service E2E failed: {error}");
        std::process::exit(1);
    }
}
