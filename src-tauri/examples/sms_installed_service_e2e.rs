...
fn bounded_output(mut child: std::process::Child, label: &str) -> Result<Output, String> {
    ...
    child
        .wait_with_output()
        .map_err(|e| format!("collect {label}: {e}"))
}
fn run_cli_with_stdin(
    ...
    if !binary.is_file() {
        return Err(format!(
            "missing prebuilt monkey-cli at {}",
            binary.display()
        ));
    }
    if std::fs::metadata(&binary).map_err(|e| e.to_string())?.len() == 0 {
        return Err(format!(
            "{} is the zero-byte build placeholder",
            binary.display()
        ));
    }
    ...
    let mut child = command
        .spawn()
        .map_err(|e| format!("start monkey-cli {args:?}: {e}"))?;
    if let Some(value) = stdin {
        child
            ...
            .map_err(|e| format!("write monkey-cli stdin: {e}"))?;
    }
...
fn create_profile() -> Result<String, String> {
    let output = require_cli(
        ...
        &[
            "profiles",
            "create",
            &format!("SMS installed-service E2E {}", unique()),
            "--json",
        ],
    )?;
    let v: serde_json::Value =
        serde_json::from_slice(&output.stdout).map_err(|e| format!("profile JSON: {e}"))?;
    v.get("id")
        .and_then(serde_json::Value::as_str)
        .map(str::to_string)
        .ok_or_else(|| format!("profile JSON had no id: {v}"))
}
...
impl ModelFixture {
    fn spawn(expected_reply: String) -> Result<Self, String> {
        let listener = std::net::TcpListener::bind(("127.0.0.1", 0))
            .map_err(|e| format!("bind model fixture: {e}"))?;
        ...
        match stream.read(&mut scratch) {
            ...
            Ok(n) => {
                ...
                if let Some(start) = header_end {
                    if received.len() >= start + content_length {
                        break;
                    }
                }
            }
...
fn write_recipe(profile: &str, workspace: &Path, model_base: &str) -> Result<(), String> {
    ...
    std::fs::write(
        recipes.join(format!("{RECIPE}.json")),
...
fn profile_state_db(profile: &str) -> Result<PathBuf, String> {
    ...
    let data = little_monkey_lib::app_paths::data_dir()
        .ok_or_else(|| "could not resolve profile data directory".to_string());
    match old {
        Some(v) => std::env::set_var(PROFILE_ENV, v),
        None => std::env::remove_var(PROFILE_ENV),
    }
...
fn find_inbound(
    ...
    while let Some(row) = rows.next().map_err(|e| e.to_string())? {
        let sender: Option<String> = row.get(2).map_err(|e| e.to_string())?;
        let Some(sender_id) = sender else { continue };
        if expected_sender.is_some_and(|expected| sender_id.as_str() != expected) {
            continue;
        }
        return Ok(Some(InboundProof {
            provider_event_id: row.get(0).map_err(|e| e.to_string())?,
            conversation_id: row.get(1).map_err(|e| e.to_string())?,
            sender_id,
            disposition: row.get(3).map_err(|e| e.to_string())?,
            ingress_id: row.get(4).map_err(|e| e.to_string())?,
...
fn wait_for_service_pid(profile: &str) -> Result<u64, String> {
    ...
    Err(format!(
        "installed daemon never reached a fresh heartbeat; last status: {last}"
    ))
}
fn wait_for_inbound(
    db: &Path,
    ...
    while Instant::now() < deadline {
        if let Some(proof) = find_inbound(db, account, marker, expected_sender)? {
            return Ok(proof);
        }
        std::thread::sleep(Duration::from_millis(500));
    }
    Err(format!(
        "no real SMS inbound containing {marker:?} arrived within {}s",
        PROVIDER_WAIT.as_secs()
    ))
}
fn wait_for_outbound(
    db: &Path,
    account: &str,
    conversation: &str,
    expected: &str,
    ...
    while Instant::now() < deadline {
        if let Some(id) = find_outbound(db, account, conversation, expected)? {
            return Ok(id);
        }
        std::thread::sleep(Duration::from_millis(500));
    }
...
fn wait_for_delivery(
    ...
    while Instant::now() < deadline {
        if let Ok(output) = run_cli(
            Some(profile),
            &["telecom", "messages", account, "--limit", "50", "--json"],
        ) {
            if output.status.success() {
                ...
                if let Ok(rows) = serde_json::from_slice::<serde_json::Value>(&output.stdout) {
                    if let Some(items) = rows.as_array() {
                        for row in items {
                            let matches = row.get("direction").and_then(serde_json::Value::as_str)
                                == Some("outbound")
                                && row.get("peer_number").and_then(serde_json::Value::as_str)
                                    == Some(peer)
                                && row.get("text").and_then(serde_json::Value::as_str)
                                    == Some(expected);
                            if matches {
                                if row
                                    .get("delivery_state")
                                ...
                                {
                                    return Err(format!(
                                        "carrier reported the generated SMS undelivered: {}",
                                        row.get("error").unwrap_or(&serde_json::Value::Null)
                                    ));
                                }
                            }
                        }
    ...
    Err(format!(
        "no delivered receipt for exact generated SMS within {}s; last telecom messages: {last}",
...
fn dump_diagnostics(profile: &str, account: Option<&str>) {
    for args in [
        ...
        vec!["channels", "list", "--json"],
        vec!["telecom", "list", "--json"],
    ] {
        if let Ok(out) = run_cli(Some(profile), &args) {
            eprintln!("--- {args:?} ---\n{}", output_text(&out));
        }
    }
    if let Some(account) = account {
        if let Ok(out) = run_cli(
            Some(profile),
            &["channels", "events", account, "--limit", "80", "--json"],
        ) {
            eprintln!("--- channel events ---\n{}", output_text(&out));
        }
        if let Ok(out) = run_cli(
            Some(profile),
            &["telecom", "messages", account, "--limit", "80", "--json"],
        ) {
            eprintln!("--- telecom messages ---\n{}", output_text(&out));
        }
    }
}
fn cleanup(profile: Option<&str>, account: Option<&str>) {
    if let Some(profile) = profile {
        ...
        if let Some(account) = account {
            let _ = run_cli(Some(profile), &["telecom", "remove", account]);
        }
...
impl LiveConfig {
    fn from_env() -> Result<Self, String> {
        ...
        if !matches!(carrier.as_str(), "twilio" | "telnyx" | "plivo") {
            return Err("SMS_E2E_CARRIER must be twilio, telnyx or plivo".into());
        }
        let public_base = req("SMS_E2E_PUBLIC_BASE")?
            .trim_end_matches('/')
            .to_string();
        if !public_base.starts_with("https://") {
            return Err("SMS_E2E_PUBLIC_BASE must be a public https:// origin".into());
        }
        let webhook_public_key = std::env::var("SMS_E2E_WEBHOOK_PUBLIC_KEY")
            .ok()
            .filter(|v| !v.trim().is_empty());
        if carrier == "telnyx" && webhook_public_key.is_none() {
            return Err("SMS_E2E_WEBHOOK_PUBLIC_KEY is required for Telnyx".into());
        }
        let webhook_port = std::env::var("SMS_E2E_WEBHOOK_PORT")
            .unwrap_or_else(|_| "38447".into())
            .parse::<u16>()
            .map_err(|_| "SMS_E2E_WEBHOOK_PORT must be a non-zero u16".to_string())?;
        if webhook_port == 0 {
            return Err("SMS_E2E_WEBHOOK_PORT must be non-zero".into());
        }
        Ok(Self {
            carrier,
            carrier_account_id: req("SMS_E2E_CARRIER_ACCOUNT_ID")?,
            from_number: req("SMS_E2E_FROM_NUMBER")?,
            credential: req("SMS_E2E_CREDENTIAL")?,
            public_base,
            webhook_public_key,
            webhook_port,
        })
...
async fn run_case(config: &LiveConfig) -> Result<(), String> {
    if std::env::var(REQUIRE_ENV).as_deref() != Ok("1") {
        return Err(format!(
            "refusing a billable real-carrier run unless {REQUIRE_ENV}=1"
        ));
    }
    let stamp = unique();
    ...
    if result.is_err() {
        if let Some(p) = profile.as_deref() {
            dump_diagnostics(p, account.as_deref());
        }
    }
    cleanup(profile.as_deref(), account.as_deref());
    result
}
...
async fn main() {
    let config = match LiveConfig::from_env() {
        Ok(v) => v,
        Err(e) => {
            eprintln!("SMS installed-service E2E configuration error: {e}");
            std::process::exit(2);
        }
    };
    if let Err(e) = run_case(&config).await {
        eprintln!("SMS installed-service E2E failed: {e}");
        std::process::exit(1);
    }
}
