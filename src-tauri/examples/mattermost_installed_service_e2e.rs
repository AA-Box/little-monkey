...
//!
//! The server and both users are real. The harness configures Little Monkey
//! only through the production CLI, writes the bot session token through
//! `channels set-token`, installs the real user service, then has a second
//! Mattermost user send and observe the reply through Mattermost's REST API.
//!
//! The sole deterministic component is an OpenAI-compatible model origin
//! reached through an ordinary recipe `target.local_url`. It cannot create
//! channel events, write an outbox row, or post to Mattermost.
...
async fn run_case(
    ...
    let result = (|| -> Result<(), String> {
        ...
        // Production secret path: stdin -> KeyringChannelSecrets -> native
        // Secret Service. The installed daemon later resolves the same entry.
        require_cli_stdin(
            &created,
...
async fn main() {
    let base_url = std::env::var("MM_E2E_BASE_URL").unwrap_or_else(|_| "http://localhost:8065".to_string());
    let bot_token = std::env::var("MM_E2E_BOT_TOKEN")
        .unwrap_or_else(|_| String::new());
    let external_token = std::env::var("MM_E2E_EXTERNAL_TOKEN")
        .unwrap_or_else(|_| String::new());
    let channel_id = std::env::var("MM_E2E_CHANNEL_ID")
        .unwrap_or_else(|_| String::new());
