//! Real Teams user -> Microsoft-signed Bot Framework Activity -> installed
//! resident daemon -> production agent -> Bot Framework reply -> independent
//! Teams-client observation.
//!
//! This process never constructs the Teams adapter and never fabricates an
//! Activity. The external user is the only sender/observer boundary that is
...
const REQUIRE_ENV: &str = "LITTLE_MONKEY_REQUIRE_TEAMS_INSTALLED_SERVICE_E2E";
const RECIPE: &str = "teams-installed-service-e2e";
const REPLY_PREFIX: &str = "little-monkey teams installed-service reply";
...
fn create_profile() -> Result<String, String> {
    let name = format!("Teams installed-service E2E {}", unique());
...
fn add_account(profile: &str, app_id: &str, tenant_id: &str) -> Result<String, String> {
    let label = format!("teams-e2e-{}", unique());
...
fn write_recipe(profile: &str, workspace: &Path, model_base: &str) -> Result<(), String> {
    ...
    let recipe = serde_json::json!({
        "version": 1,
        "name": RECIPE,
        "target": { "local_url": model_base, "model": "teams-e2e-fixture" },
...
fn find_inbound(
    ...
    connection
        .query_row(
            "SELECT provider_event_id, conversation_id, ingress_id, job_id
             FROM channel_events
             WHERE account_id = ?1
        ...
        .optional()
        .map_err(|error| format!("read exact durable Teams inbound: {error}"))
}
...
fn find_outbound(
    ...
    connection
        ...
        .optional()
        .map_err(|error| format!("read exact durable Teams outbound: {error}"))
}

fn durable_reference_exists(
    db: &Path,
    account_id: &str,
    conversation_id: &str,
) -> Result<bool, String> {
    ...
    let found: Option<i64> = connection
        .query_row(
            "SELECT 1 FROM channel_conversation_references
             WHERE account_id = ?1 AND conversation_id = ?2 LIMIT 1",
            (account_id, conversation_id),
            |row| row.get(0),
        )
        .optional()
        .map_err(|error| format!("read durable Teams conversation reference: {error}"))?;
    Ok(found.is_some())
}
...
impl LiveConfig {
    fn from_env() -> Result<Self, String> {
        ...
        let public_base = required("TEAMS_E2E_PUBLIC_BASE")?
            .trim_end_matches('/')
            .to_string();
        if !public_base.starts_with("https://") {
            return Err("TEAMS_E2E_PUBLIC_BASE must be a public https:// origin".to_string());
        }
        let webhook_port = std::env::var("TEAMS_E2E_WEBHOOK_PORT")
            .unwrap_or_else(|_| "38446".to_string())
            .parse::<u16>()
            .map_err(|_| "TEAMS_E2E_WEBHOOK_PORT must be a non-zero u16".to_string())?;
        if webhook_port == 0 {
            return Err("TEAMS_E2E_WEBHOOK_PORT must be non-zero".to_string());
        }
        Ok(Self {
            app_id: required("TEAMS_E2E_APP_ID")?,
            tenant_id: required("TEAMS_E2E_TENANT_ID")?,
            app_password: required("TEAMS_E2E_APP_PASSWORD")?,
            public_base,
            external_sender_id: required("TEAMS_E2E_EXTERNAL_SENDER_ID")?,
            destination: required("TEAMS_E2E_DESTINATION")?,
            webhook_port,
        })
    }
}
...
async fn run_case(config: &LiveConfig) -> Result<(), String> {
    if std::env::var(REQUIRE_ENV).as_deref() != Ok("1") {
        return Err(format!(
            "refusing to install a real OS service and use live Teams credentials unless {REQUIRE_ENV}=1"
        ));
    }

    let stamp = unique();
    let marker = format!("lm-teams-installed-{stamp:x}");
    ...
    let result: Result<(), String> = async {
        ...
        let workspace = std::env::temp_dir().join(format!("lm-teams-e2e-workspace-{stamp:x}"));
        std::fs::create_dir_all(&workspace)
            .map_err(|error| format!("create workspace {}: {error}", workspace.display()))?;
        write_recipe(&created, &workspace, &model.base)?;

        let account = add_account(&created, &config.app_id, &config.tenant_id)?;
        account_id = Some(account.clone());
        let credential = serde_json::json!({ "app_password": config.app_password }).to_string();
        require_cli_stdin(
            &created,
        ...
        eprintln!(
            "\nTeams messaging endpoint for this isolated run:\n\n    {callback_url}\n\nPoint the disposable/test Azure Bot messaging endpoint at that exact URL. After the provider configuration is active, press Enter here.\n"
        );
        ...
        let inbound = loop {
            if let Some(proof) = find_inbound(&db, &account, &config.external_sender_id, &marker)? {
                break proof;
            }
        ...
        if !durable_reference_exists(&db, &account, &inbound.conversation_id)? {
            return Err(format!(
                "authenticated Teams inbound {} was accepted without its durable conversation reference",
                inbound.provider_event_id
            ));
        }
        eprintln!(
            "Teams inbound {} in conversation {} became ingress {} / job {} with durable addressing",
            inbound.provider_event_id, inbound.conversation_id, inbound.ingress_id, inbound.job_id
        );
        ...
        let requests = model.requests();
        if !requests.iter().any(|request| request.contains(&marker)) {
            return Err("installed agent never sent the real Teams marker to the model".to_string());
        }
        if !requests
            .iter()
            .any(|request| request.contains(r#"\"name\":\"send_message\""#))
        ...
        let provider_id = loop {
            if let Some(provider_id) =
                find_outbound(&db, &account, &inbound.conversation_id, &expected_reply)?
            {
                break provider_id;
            }
            if Instant::now() >= outbound_deadline {
                return Err(format!(
                    "generated Teams reply never became a provider-named durable outbound event: {expected_reply:?}"
                ));
            }
            tokio::time::sleep(Duration::from_millis(500)).await;
        };
        if provider_id.starts_with("local:") || provider_id.trim().is_empty() {
            return Err(format!("Bot Framework did not name the generated reply: {provider_id:?}"));
        }
        eprintln!("Bot Framework accepted generated reply as activity {provider_id}");

        eprintln!(
            "\nOn that same independent Teams client, wait for the bot reply and copy/paste the exact received text here.\nExpected prefix: {REPLY_PREFIX}\n"
        );
        let mut observed = String::new();
        io::stdin()
            .read_line(&mut observed)
            .map_err(|error| format!("read external Teams observation: {error}"))?;
        let observed = observed.trim_end_matches(&['\r', '\n'][..]);
        if observed != expected_reply {
            return Err(format!(
                "external Teams observation did not exactly match generated reply; expected {expected_reply:?}, got {observed:?}"
            ));
        }
        eprintln!("independent Teams client observed the exact generated reply");
        Ok(())
    }
    .await;
...
async fn main() {
    let config = match LiveConfig::from_env() {
        Ok(config) => config,
        Err(error) => {
            eprintln!("Teams installed-service E2E configuration error: {error}");
            std::process::exit(2);
        }
    };
    if let Err(error) = run_case(&config).await {
        eprintln!("Teams installed-service E2E failed: {error}");
        std::process::exit(1);
    }
}
