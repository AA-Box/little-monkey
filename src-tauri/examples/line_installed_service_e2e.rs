//! Real LINE user -> LINE signed webhook -> installed resident daemon -> agent
//! -> LINE push API -> independent user observation acceptance.
//!
//! The harness never calls the LINE adapter directly. The only interactive
//! boundary is the independent LINE user: LINE exposes no API that can read a
//! consumer's received messages or emit delivery receipts for a push, so the
//! observer copies the exact received reply back to this process.
...
const REQUIRE_ENV: &str = "LITTLE_MONKEY_REQUIRE_LINE_INSTALLED_SERVICE_E2E";
const RECIPE: &str = "line-installed-service-e2e";
const REPLY_PREFIX: &str = "little-monkey line installed-service reply";
...
fn create_profile() -> Result<String, String> {
    let name = format!("LINE installed-service E2E {}", unique());
...
fn add_account(profile: &str) -> Result<String, String> {
    let label = format!("line-e2e-{}", unique());
    let output = require_cli(
        Some(profile),
        &["channels", "add", "line", &label, "--json"],
    )?;
    let payload: serde_json::Value = serde_json::from_slice(&output.stdout)
...
fn write_recipe(profile: &str, workspace: &Path, model_base: &str) -> Result<(), String> {
    ...
    let recipe = serde_json::json!({
        "version": 1,
        "name": RECIPE,
        "target": { "local_url": model_base, "model": "line-e2e-fixture" },
...
fn find_inbound(
    ...
    connection
        ...
        .optional()
        .map_err(|error| format!("read exact durable LINE inbound: {error}"))
}
...
fn find_outbound(
    ...
    connection
        ...
        .optional()
        .map_err(|error| format!("read exact durable LINE outbound: {error}"))
}
...
async fn line_webhook_state(
    client: &reqwest::Client,
    access_token: &str,
) -> Result<WebhookState, String> {
    let response = client
        .get(format!("{LINE_API_BASE}/v2/bot/channel/webhook/endpoint"))
        .bearer_auth(access_token)
        .send()
        .await
        .map_err(|error| format!("read LINE webhook endpoint: {error}"))?;
...
async fn set_line_webhook(
    client: &reqwest::Client,
    access_token: &str,
    endpoint: &str,
) -> Result<(), String> {
    let response = client
        .put(format!("{LINE_API_BASE}/v2/bot/channel/webhook/endpoint"))
        .bearer_auth(access_token)
        .json(&serde_json::json!({ "endpoint": endpoint }))
        .send()
        .await
        .map_err(|error| format!("set LINE webhook endpoint: {error}"))?;
...
async fn test_line_webhook(
    client: &reqwest::Client,
    access_token: &str,
    endpoint: &str,
) -> Result<(), String> {
    let response = client
        .post(format!("{LINE_API_BASE}/v2/bot/channel/webhook/test"))
        .bearer_auth(access_token)
        .json(&serde_json::json!({ "endpoint": endpoint }))
        .send()
        .await
...
impl LiveConfig {
    fn from_env() -> Result<Self, String> {
        ...
        let public_base = required("LINE_E2E_PUBLIC_BASE")?
            .trim_end_matches('/')
            .to_string();
        if !public_base.starts_with("https://") {
            return Err("LINE_E2E_PUBLIC_BASE must be a public https:// origin".to_string());
        }
        let webhook_port = std::env::var("LINE_E2E_WEBHOOK_PORT")
            .unwrap_or_else(|_| "38444".to_string())
            .parse::<u16>()
            .map_err(|_| "LINE_E2E_WEBHOOK_PORT must be a non-zero u16".to_string())?;
        if webhook_port == 0 {
            return Err("LINE_E2E_WEBHOOK_PORT must be non-zero".to_string());
        }
        Ok(Self {
            channel_secret: required("LINE_E2E_CHANNEL_SECRET")?,
...
async fn run_case(config: &LiveConfig) -> Result<(), String> {
    if std::env::var(REQUIRE_ENV).as_deref() != Ok("1") {
        return Err(format!(
            "refusing to install a real OS service and use a live LINE channel unless {REQUIRE_ENV}=1"
        ));
    }
    let stamp = unique();
    let marker = format!("lm-line-installed-{stamp:x}");
    let expected_reply = format!("{REPLY_PREFIX} {marker}");
    let model = ModelFixture::spawn(marker.clone())?;
    let client = reqwest::Client::builder()
        .timeout(Duration::from_secs(30))
        .build()
        .map_err(|error| format!("build LINE control client: {error}"))?;
    let mut profile: Option<String> = None;
    let mut account_id: Option<String> = None;
    let mut previous_webhook: Option<WebhookState> = None;

    let result: Result<(), String> = async {
        let created = create_profile()?;
        profile = Some(created.clone());
        let workspace = std::env::temp_dir().join(format!("lm-line-e2e-workspace-{stamp:x}"));
        ...
        let before = line_webhook_state(&client, &config.channel_access_token).await?;
        if !before.active {
            return Err(
                "LINE channel has webhook delivery disabled; enable Use webhook in the LINE Developers Console before this live acceptance"
                    .to_string(),
            );
        }
        if config.mutate_webhook {
            previous_webhook = Some(before.clone());
            set_line_webhook(&client, &config.channel_access_token, &callback_url).await?;
            let deadline = Instant::now() + Duration::from_secs(90);
            loop {
                let state = line_webhook_state(&client, &config.channel_access_token).await?;
                if state.endpoint == callback_url && state.active {
                    break;
                }
                if Instant::now() >= deadline {
                    return Err(format!(
                        "LINE never reported the new callback {callback_url:?}; last state {state:?}"
                    ));
                }
                tokio::time::sleep(Duration::from_secs(1)).await;
            }
        } else if before.endpoint != callback_url {
            return Err(format!(
                "LINE webhook is configured for {:?}, not {:?}; preconfigure the exact generated callback or set LINE_E2E_MUTATE_WEBHOOK=1 on a disposable channel",
                before.endpoint, callback_url
            ));
        }
        ...
        let inbound = loop {
            if let Some(proof) = find_inbound(&db, &account, &config.external_user_id, &marker)? {
                break proof;
            }
        ...
        eprintln!(
            "LINE inbound {} became ingress {} / job {}",
        ...
        let requests = model.requests();
        if !requests.iter().any(|request| request.contains(&marker)) {
            return Err("installed agent never sent the real LINE marker to the model".to_string());
        }
        if !requests
            .iter()
            .any(|request| request.contains(r#""name":"send_message""#))
        ...
        let provider_id = loop {
            if let Some(provider_id) =
                find_outbound(&db, &account, &config.external_user_id, &expected_reply)?
            {
                break provider_id;
            }
            if Instant::now() >= outbound_deadline {
                return Err(format!(
                    "generated LINE reply never became a provider-named durable outbound event: {expected_reply:?}"
                ));
            }
            tokio::time::sleep(Duration::from_millis(500)).await;
        };
        if provider_id.starts_with("local:") || provider_id.trim().is_empty() {
            return Err(format!("LINE did not name the generated push: {provider_id:?}"));
        }
        eprintln!("LINE accepted generated reply as message {provider_id}");

        eprintln!(
            "\nOn that same independent LINE client, wait for the reply and copy/paste the exact received text here.\nExpected prefix: {REPLY_PREFIX}\n"
        );
        let mut observed = String::new();
        io::stdin()
            .read_line(&mut observed)
            .map_err(|error| format!("read external LINE observation: {error}"))?;
        let observed = observed.trim_end_matches(['\r', '\n']);
        if observed != expected_reply {
            return Err(format!(
                "external LINE observation did not exactly match generated reply; expected {expected_reply:?}, got {observed:?}"
            ));
        }
        eprintln!("independent LINE client observed the exact generated reply");
        Ok(())
    }
    .await;
...
async fn main() {
    let config = match LiveConfig::from_env() {
        Ok(config) => config,
        Err(error) => {
            eprintln!("LINE installed-service E2E configuration error: {error}");
            std::process::exit(2);
        }
    };
    if let Err(error) = run_case(&config).await {
        eprintln!("LINE installed-service E2E failed: {error}");
        std::process::exit(1);
    }
}
