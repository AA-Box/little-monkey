...
fn cli() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_monkey-cli"))
}
...
fn run_boundary_child(
    ...
    let test_name = match mode {
        "writer" => "native_secure_store_writer_child",
        "reader" => "native_secure_store_reader_child",
                    ...
                    if service_running && heartbeat_fresh && pid != u64::from(std::process::id()) {
                        ...
                        if matches!(health, "degraded" | "connected") {
                            ...
                            if rendered.contains(secret) {
                                return Err("the Telegram credential leaked into channel health/status".to_string());
                            }
                            return Ok(());
                        }
                    }
...
fn native_secure_store_writer_child() {
    ...
    assert!(secret.len() <= 8192, "writer credential exceeded desktop bound");
...
fn native_secure_store_reader_child() {
    ...
    assert_eq!(exact_digest(&secret), expected, "native credential digest mismatch");
}
