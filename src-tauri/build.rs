fn main() {
    ...
    // `examples`/`tests`/`benches` never get the Common-Controls-v6 manifest,
    // so any of them that construct a real window/webview on Windows+MSVC
    // crash on launch with STATUS_ENTRYPOINT_NOT_FOUND (0xc0000139) — see
    ...
    // binary's existing manifest embedding is left untouched).
    ...
    if target_os == "windows" && target_env == "msvc" {
        ...
        for kind in ["examples", "tests", "benches"] {
