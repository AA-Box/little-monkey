...
- Work across eight right-sidebar tabs — code review, single-file diff, terminal, in-app browser, side tasks, workspace files, background tasks, and processes — opened as chips that all stay mounted, so switching tabs never loses state. A region-wide fullscreen toggle and one shared, drag-resizable, persisted width apply to the whole tab strip, and every tab has its own keyboard shortcut on every platform.
- Review changes in a git-backed panel (real `git` porcelain output) listing every changed file with a per-file diff view, PR-aware. One panel serves both review tabs: pick the base — the branch's merge-base with its upstream ("what would this PR contain") or HEAD ("what is uncommitted") — and the layout — every file's diff stacked in one scroll, or one selected file at a time. The `Review` tab opens on the stacked view and the `Diff` tab on the single-file one, each with its own shortcut, and either can be switched from the toolbar. Against HEAD the file list comes from uncapped porcelain output and each file's diff loads when you open it, so a working tree with more changed files than one review payload carries is still fully browsable; against the merge-base the panel is bounded by that payload's 300-file cap and says so.
