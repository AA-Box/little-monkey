...
- Declared memory and wall-clock limits still are not enforced as per-run OS budgets: there is no cgroup leg, and the Windows live-shell job uses fixed safety ceilings rather than K4's declared limit semantics. What exists is a userspace watchdog over daemon jobs plus per-tool timeouts. The daemon memory budget is opt-in and the wall-clock default is seven days, so both are effectively off unless a job asks for them.
- On Unix the generic kernel-held shell baseline still refuses only core dumps. File-size and descriptor ceilings are implemented and tested but unset because legitimate builds can produce large artifacts; CPU time is deliberately uncapped because it accumulates per core, and the relevant memory rlimit is a no-op on macOS. Windows live shells instead inherit the sandbox job object's fixed 512-process and 4 GiB tree ceilings; those are containment guardrails, not the missing K4 per-class budgets.
