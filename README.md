...
- Seed each kind's declared bounds from its class, then retain enforced daemon-job and browser-session overrides from their owners, so `monkey processes show` reports both defaults and owner policy. Generic admission can still accept unsupported overrides; K4 remains open until those are rejected or reported as unavailable.
...
- Declared memory limits are not portable per-run OS budgets: there is no cgroup leg, and the Windows live-shell job uses fixed safety ceilings rather than `ProcessLimits` semantics. What exists is a sampling watchdog over daemon jobs, a browser-session watchdog, a six-hour configurable default for the four WebView kinds, and per-tool timeouts. Daemon memory is opt-in; its seven-day wall bound is always present but deliberately loose.
- On Unix the generic kernel-held shell baseline still refuses only core dumps. File-size and descriptor ceilings are implemented and tested but unset because legitimate builds can produce large artifacts; CPU time is deliberately uncapped because it accumulates per core, and the relevant memory rlimit is a no-op on macOS. Windows live shells instead inherit the sandbox job object's fixed 512-process and 4 GiB tree ceilings. These are accurately named platform guardrails; Linux per-class cgroups, `ProcessLimits`-backed Windows jobs, and a macOS kernel-owned process tree remain explicitly deferred stronger capabilities, not portable K4 promises.
...
- Desktop foreground/background shell output is drained concurrently and only the configured cap is retained; monkey-cli still collects complete stdout/stderr until its timeout. The desktop cap bounds app memory, not how many bytes the child may emit before timeout or process-tree termination.
