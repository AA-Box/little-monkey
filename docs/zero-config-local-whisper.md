...
The Whisper engine itself is compiled into the desktop application, for a fixed CPU baseline rather than for whichever machine built the release: AVX2 with FMA, F16C and BMI2 on x86_64 (Intel Haswell, AMD Excavator/Zen and later), ARMv8.2 with dot product and FP16 arithmetic on arm64 (every Apple silicon Mac and Windows on Arm device, Raspberry Pi 5, Graviton 2 and later; not Raspberry Pi 4 or other Cortex-A72-and-older boards). The baseline lives in `.cargo/config.toml`, and the local-speech workflow fails any release target whose build departs from it. The same implementation is used by Talk, desktop companion transcription, phone-call transcription, and paired-device voice input. Browser `MediaRecorder` WebM/Opus recordings are demuxed and decoded in-process before Whisper inference; WAV and the other formats supported by the bundled audio decoder use the same path.

A CPU below the baseline whisper.cpp is compiled for — x86_64 without AVX2 (pre-Haswell Intel, Atom-class Celeron, Pentium Silver and Jasper Lake parts), or arm64 without ARMv8.2 dot product (Raspberry Pi 4 and other Cortex-A72-and-older boards) — would execute an instruction it does not have, and because whisper.cpp runs in-process that is SIGILL for the whole application. So the app checks the running CPU before it downloads or loads a model: local transcription fails with "Built-in transcription needs a CPU with AVX2 (x86_64) or ARMv8.2 dot product (arm64)", Talk and the telephony surface report themselves unready, and a hosted-provider or executable-extension transcription backend still works. The instruction list checked in `src-tauri/src/local_whisper.rs` mirrors the `GGML_*` baseline in `.cargo/config.toml`; the two change together.
