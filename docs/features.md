...
- Connect a work account in Settings → Connectors: eleven providers (Google Drive, Microsoft Graph/SharePoint/OneDrive, Linear, Asana, Dropbox, Box, Airtable, Zendesk, HubSpot, Discord, GitLab.com and internet-reachable self-hosted GitLab) over an authorization-code flow on a loopback redirect — with PKCE for the eight that document it — using an OAuth app you register once per provider; no client credentials ship in this binary. Slack, Notion and Jira/Confluence take a pasted token, GitHub rides the `gh` CLI, and S3/R2 takes access keys. Every account is proven with one live read-only identity call before it is saved, access and refresh tokens live in the OS keychain only, an expired access token refreshes on next use behind a per-account lock within each process (nothing refreshes in the background), and removing an account makes a best-effort revocation at the provider wherever the provider publishes a callable endpoint (refreshing the token first, and sending the refresh token where the endpoint takes one, so a long-idle account still revokes). The account records identity and granted scopes; no feature reads provider data through it yet. `monkey connectors list|reverify|remove` covers the same catalog from the terminal. See [BYO OAuth clients](byo-oauth-clients.md).
