...
- Reach an agent from your own mailbox: IMAP in, SMTP out, implicit TLS on both legs with no cleartext configuration available — ports 143 and 25 are refused at construction rather than downgraded. A reply carries `In-Reply-To` and `References` so it lands in the same thread, and attachments move in both directions under the account's own size cap. One conversation per correspondent address, so a reply goes to the sender alone and never to everyone on a thread; automated mail — autoresponders, bounces and mailing-list posts — is refused before it can become a turn at all; and the mailbox is polled about every thirty seconds rather than held open with IDLE.
...
- Open a chat page the resident daemon serves on its own TLS listener, beside the remote controller's own assets and outside the signed device plane. A first-time visitor is answered with a pairing code to give you, exactly like a stranger on any other provider; loopback is the default, and a listener bound to a wildcard or multicast address, or to port zero, is refused rather than served. No attachments, and a visitor reads only their own conversation.
