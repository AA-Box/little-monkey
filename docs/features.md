...
- Manage every remembered fact in Memory Studio: pin one so it is folded into the prompt first and is exempt from expiry and the per-scope cap, merge several into one that keeps its originals' ids (they are retired, not deleted, and the merge can be undone), set an expiry date after which a fact stops reaching prompts and can be purged, and read each fact's provenance — who saved it, the chat turn it came from, what it was merged from, and when a prompt was last built from it. A disabled, expired or merge-retired fact is excluded at the one function both the desktop app and the CLI compose their prompts through, from the next prompt built — a turn already queued to the background daemon replays the prompt frozen when it was queued. `monkey memory list|pin|unpin|expire|merge|unmerge|purge` does the lifecycle half from the CLI against the same file; adding, editing, enabling, disabling, deleting, import and export stay in the desktop panel.
...
- Start from a signed first-party catalog of six skills (review, testing, documentation, browser QA, release preparation, knowledge workflows) plus declarative GitHub, GitLab, WebDAV, and REST/webhook connector packages — package definitions in Ecosystem, a different surface from the connected accounts in Settings → Connectors described below.
...
- Connect a work account in Settings → Connectors: eleven providers (Google Drive, Microsoft Graph/SharePoint/OneDrive, Linear, Asana, Dropbox, Box, Airtable, Zendesk, HubSpot, Discord, GitLab.com and internet-reachable self-hosted GitLab) over an authorization-code flow on a loopback redirect — with PKCE for the eight that document it — using an OAuth app you register once per provider (Microsoft Graph and Airtable as public clients with no secret); no client credentials ship in this binary. Slack, Notion and Jira/Confluence take a pasted token, GitHub rides the `gh` CLI, and S3/R2 takes access keys. Every account is proven with one live read-only identity call before it is saved, access and refresh tokens live in the OS keychain only, an expired access token refreshes on next use behind a per-account lock within each process (nothing refreshes in the background), and removing an account makes a best-effort revocation at the provider wherever the provider publishes a callable endpoint (refreshing the token first, and sending the refresh token where the endpoint takes one, so a long-idle account still revokes). The account records identity and the scopes the provider returned; when a provider returns no scope list the row records the scopes that were *requested*, labelled as such, rather than claiming a grant the provider never stated. No feature reads provider data through it yet. `monkey connectors list|reverify|remove` covers the same catalog from the terminal. See [BYO OAuth clients](byo-oauth-clients.md).
...
- Hold a spoken conversation in **Talk**. The default **Classic pipeline** remains the existing push-to-talk/continuous path: local VAD, configured STT, an ordinary durable agent turn, then configured TTS. The opt-in **Realtime WebRTC** engine opens one direct OpenAI audio session while keeping the saved chat transcript and Little Monkey tool/permission boundary authoritative. The modes are explicit and never silently fall back to one another. A second realtime provider, `loopback`, is selectable for acceptance runs only — a local test peer that makes no network call and answers nothing — and OpenAI stays the default. See [Desktop realtime voice](realtime-voice.md).
- Choose the microphone and speaker for either engine. Classic pipeline exposes transcription, speech, local VAD, wake phrase, and per-turn pipeline latency. Realtime separately exposes provider status, model, voice, semantic VAD/manual turns, and its own bounded connection/first-audio/end-to-end/interruption/reconnect metrics. Metrics contain no audio, transcript, secret, or tool payload.
- Put the microphone and the speaker in different places. One Talk conversation routes its input and its output endpoints independently — a local device or a paired phone for either, and not necessarily the same phone — without becoming a second chat, model context, permission boundary or pairing identity. The route is conversation-scoped and carries a generation, so moving an endpoint mid-conversation retires the previous one before the new one becomes current instead of letting stale audio arrive late. Input requires effective `voice_stream`, output requires effective `audio_playback`, and both engines can be routed: Pipeline runs host STT and TTS around the ordinary durable turn, while Realtime relays paired audio as 24 kHz PCM16 through a loopback bridge to the desktop session that holds the provider connection — never through Whisper. See [Voice Everywhere](voice-everywhere.md).
...
- Opt in to a real **wake word**, off by default: AudioWorklet PCM is resampled
  and streamed through bounded memory to a bundled native sherpa-onnx keyword
  spotter. While armed, Whisper does not run, no turn is created, and passive
...
- Talk's desktop transcription publishes no audio artifact. Passive wake PCM
  ...
  transcription call. Paired-phone Talk has its separate bounded retry store
  and foreground socket contract.
