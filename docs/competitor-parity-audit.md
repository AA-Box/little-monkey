...
It compares Little Monkey's **actually shipped** capabilities (as documented in `README.md`, `docs/features.md`, and `docs/limitations.md`) against public capability claims for fifteen named competitors, across ten gap categories. It is meant to be re-run periodically, not treated as a one-time snapshot. The 2026-07-16 revision is the evidence for that: three of its findings — no mobile app, no in-app terminal, no desktop control — were falsified by shipped work within seven weeks, and one of those categories (PC control) changed a second time: it went from "no named competitor creates competitive pressure" to three competitors documenting it.

## Methodology and honesty rules

- **Little Monkey column**: grounded in `README.md`, `docs/features.md`, and `docs/limitations.md` as they stand on `develop`, plus the docs those files themselves link where a capability lives in one of them. `ROADMAP.md` is consulted only for the linkage line. It holds future work, so an absent item means this audit found no open roadmap commitment for the capability — not that the capability exists. Items marked *(built)* remain in the file for their stated remainder. Two exceptions are made and flagged in place, where those files are silent on a fact this audit would otherwise have to guess at: the connector catalog's provider list, cited from `src-tauri/src/connectors.rs`; and the named Dispatch policies plus the routing-versus-Privacy-Firewall order, taken from `ROADMAP.md`'s own **Shipped:** heading. Both are attributed at the point of use and listed in Sources.
...
One consequence of the second rule is worth stating plainly: most competitors here are scored from a single landing page or README, so several categories read heavily "unclear / not publicly documented" — model comparison worst at 12 of fifteen rows, then PC control at 9, then memory, mobile and governance at 5 each. That marker means the source consulted does not say, not that the vendor lacks the capability.
...
| Hermes Agent | Local-first agent runtime + messaging gateway (Python) | Nous Research's agent with a self-improving skills loop, FTS5 session memory plus Honcho user modelling, seven terminal backends, and a gateway onto five messaging platforms plus CLI, Email and Home Assistant. MIT, 240.6k stars read 2026-09-03 | [github.com/nousresearch/hermes-agent](https://github.com/nousresearch/hermes-agent) |
...
| Jan | Local-first offline chat app | Offline desktop app for local models with MCP connectors, a local API server, and the self-hosted Tokamak router for model switching; a separately distributed Jan Agent is named on the docs home as *Coming soon* | [jan.ai/docs](https://www.jan.ai/docs) |
...
| Replit Agent | Cloud IDE + autonomous builder | Browser-hosted IDE whose Agent 3 (announcement published 2025-09-10) "runs on its own for up to 200 minutes", tests and fixes the app it builds in an in-pane browser preview, and can build other agents and automations | [replit.com/blog — Agent 3](https://replit.com/blog/introducing-agent-3-our-most-autonomous-agent-yet) |
...
| OpenAI Codex desktop app (in the ChatGPT app) | Desktop coding agent | Codex inside the ChatGPT desktop app; the surface the documentation maps features to by name — the in-app browser ("from the ChatGPT desktop app"), the integrated terminal, and computer use | [learn.chatgpt.com/codex](https://learn.chatgpt.com/codex) |
...
Rows 1-8 are the local-first / self-hosted block, 9-13 the hosted and CLI coding agents, 14-15 the desktop-app agents. Agent-Zoey keeps its row despite being an alpha with 24 stars: it is the closest architectural neighbour in this set — a local-first Rust runtime over Ollama and llama.cpp — so it is scored as a design comparison, not as a market threat. Rows 13 and 14 are two surfaces of one OpenAI product, kept separate because the documentation locates three features on the desktop app by name — the in-app browser, the integrated terminal, and computer use — so categories 3 and 10 would otherwise silently merge two different things. Elsewhere the source maps very few features to a named surface, and wherever it does not, both rows carry the same verdict and say so rather than inventing a difference.
...
| Jan | No | Jan Agent, "the standalone agent, distributed separately — launch autonomous coding agents against a local model", is named on the docs home but marked *Coming soon*, so nothing autonomous ships today |
...
| Replit Agent | Unclear | Not mentioned in the Agent 3 announcement; the absence is in a 2025-09-10 post, not in current Replit documentation |
...
| OpenAI Codex (CLI, IDE extension, cloud) | Unclear (terminal) / Unclear (browser) | Both surfaces are mapped to the desktop app: "Each chat in the ChatGPT desktop app includes a terminal scoped to its current project or worktree." Whether these surfaces carry their own in-app terminal or browser is unclear / not publicly documented |
| OpenAI Codex desktop app | Yes | Surface-mapped on the page: "Use your browser: Work in Edge, Brave, Opera, or Vivaldi as well as Chrome from the ChatGPT desktop app", alongside the integrated terminal |
...
| Competitor | Support | Notes |
| --- | --- | --- |
| Agent-Zoey / Zoey | Partial | A Provider Router "routes LLM requests with load balancing, fallbacks, and cost optimization" — cost-optimized routing, load balancing and fallback chains are configuration, not a documented task-class policy engine; Alpha |
...
**Little Monkey status:** Shipped at the mechanism level. `docs/features.md` describes chat against managed `llama.cpp`, Ollama, MLX or configured cloud/BYOK providers "with capability-aware routing, provider failover, context compaction, usage accounting, and rate-limit warnings". A user-authored routing-policy surface is shipped too: `ROADMAP.md`'s first item records named Dispatch policies in **Settings → Automation → Dispatch policies** under its **Shipped:** heading, and adds that routing runs before the Privacy Firewall, which still overrides it. Both facts are the flagged `ROADMAP.md` exception — the surface appears in none of the three shipped-behaviour files, so — as with Team Mode — this audit records that boundary.

**Risk if open:** Low — capability-aware routing, failover and named Dispatch policies are all shipped, and no competitor documents a task-class, cost, latency or sensitivity policy engine the user authors: AnythingLLM's Model Router and Zoey's Provider Router are the closest, and neither is documented as user-authored precedence rules. Most of what is left is documentation — the policy surface exists and none of the three grounding files mentions it — on top of the three capability remainders named in the roadmap linkage below.
...
| Hermes Agent | Yes | Gateway onto "Telegram, Discord, Slack, WhatsApp, Signal, and CLI" plus Email and Home Assistant; 40+ tools; full MCP integration "compatible with the agentskills.io open standard". A "Community WeChat bridge" is linked from the README as a third-party project, not an official integration |
| OpenClaw | Yes | Seven channels named in the README — WhatsApp, Telegram, Slack, Discord, Google Chat, Signal, iMessage — followed by "and other messaging services", so the full list is unclear / not publicly documented; plus ClawHub skills and a plugin SDK |
...
| AnythingLLM | Yes | "A full AI assistant in your pocket — running on-device with no cloud, no API keys, and no limits", syncing "chats, threads, tools, and even run prompts across devices, all on your local network". The page offers Google Play and a direct APK and names no iOS build, so iOS support is unclear / not publicly documented |
...
| OpenAI Codex (CLI, IDE extension, cloud) | Unclear | The dedicated computer-use page places the feature in the ChatGPT desktop app only, enabled through **Plugins > Computer Use** with OS-level per-app approval, and describes no CLI, IDE or cloud variant — silence about the other surfaces, not a documented absence |
| OpenAI Codex desktop app | Yes | "In supported regions, Computer Use in the ChatGPT desktop app is available on macOS and Windows with ChatGPT Work and Codex" — installed from **Plugins > Computer Use**, needing macOS Screen Recording and Accessibility, with per-app consent ("ChatGPT asks for your permission before it can use an app on your computer") and an **Always allow** option. The page scopes availability regionally without enumerating the exclusions |
...
**Risk if open:** Medium, up from Low. The 2026-07-16 claim that no named competitor documents desktop control is dead: Claude Desktop documents it in detail, the Codex desktop app documents it on macOS and Windows behind a ChatGPT Work or Codex plan and a regional gate (the page maps it to that surface only; whether the other Codex surfaces have it is unclear / not publicly documented), AnythingLLM lists an "AI Computer use" beta preview, and OpenClaw's companion nodes reach a device's screen. The risk is no longer absence of the capability but the unfinished release gate on two of the three input backends.
...
| 4 | Memory | Shipped: `memory` agent tool, Knowledge Stacks 2.0 with end-to-end retrieval inspection, learned skills as procedural memory; Memory Studio's pin, merge, expiry and provenance lifecycle; remaining: Memory Studio still has exactly two scopes, and its expiry, purge and merge boundaries are stated in docs/limitations.md | Medium | Medium | shipped — see docs/features.md for the memory tool, Knowledge Stacks and Memory Studio |
| 5 | Model routing | Shipped: capability-aware routing and provider failover, plus named Dispatch policies (attributed to `ROADMAP.md`) in **Settings → Automation → Dispatch policies** that appear in no feature documentation; remaining: subagent task classes, managed llama.cpp for local-only policies, and recording the decision in the durable run ledger | Low | Medium | `1. Policy-driven model routing` *(partially built)* |
| 6 | Connector ecosystems | Shipped: MCP with OAuth and BYO clients, signed packages, WASM extensions, one Discover catalog, six first-party skills, and sixteen messaging providers on one ingress; remaining: the app/data catalog is five provider accounts (GitHub, Slack, Notion, Jira, S3) against the named work-app catalogs of the hosted competitors, and neither grounding file enumerates them | High | High | non-goals "No Gmail/Outlook inbox integration" and "No Google Drive knowledge connector"; otherwise shipped — see docs/features.md; no roadmap item |
...
1. **Connector ecosystem breadth still trails the leaders.** The only survivor of the 2026-07-16 top three. Claude Desktop names Calendar, Slack, GitHub, Linear and Notion behind a graphical setup flow, Codex names Gmail, Slack, GitHub, Apple Messages and Linear, and Replit names Notion, Linear, Dropbox and SharePoint; Little Monkey's own app/data catalog reaches Notion but not Calendar, and Google Drive is closed by an explicit non-goal. (Copilot ships two MCP servers by default, and Open WebUI and AnythingLLM document MCP generically without a named work-app catalog, so this rests on three named catalogs, not six.) This refresh also found that neither grounding file enumerates the catalog's five providers — `docs/features.md`'s four named connector *packages* are a different object, the skills catalog's declarative packages — so a reader cannot tell from the docs which work apps are covered. That is worth fixing before the capability gap is argued about. Note the asymmetry in the other direction too: sixteen messaging providers reach one durable ingress, more than any competitor's *named* channel list found here (OpenClaw's seven end with "and other messaging services", so its full list is unclear), and `docs/features.md` names only three of them.
2. **The autonomous run stops one step short of delivery.** The coordinator, the worktree workers, the verification and the acceptance evidence are all shipped and in places stricter than the competition. What differs is the last mile: `monkey task start` is "queued with a read/write workspace grant and network/external mutations disabled by default; delivery still requires the relevant repository policy and explicit confirmation", and GitHub delivery and merge stay outside the worker. Competitors default to opening the PR. That is a defensible boundary rather than a missing feature — but a reader of `docs/features.md` cannot find the capability at all, which is the part worth fixing.
...
Direction of travel since 2026-07-16 matters more than any single verdict here. Three of that revision's findings — no mobile app, no in-app terminal, no desktop control — were falsified by shipped work within seven weeks. One category went the other way: PC control moved from "no named competitor creates competitive pressure" to three competitors documenting it plus a fourth exposing device screens. A parity audit that is re-run twice a year would have been wrong about three of ten categories — one of them for two independent reasons — for months at a time; the cadence is the finding.

This audit proposes no `ROADMAP.md` change. The highest-priority gaps either map to a live partially-built item or fall inside a stated non-goal, and the work that shipped since the last revision correctly carries no roadmap item at all under that file's no-Done-items rule. What it does propose is documentation work in the files it is grounded in: `docs/features.md` describes no Autonomous Task, Team Mode, or routing-policy surface and carries no messaging-channel roster, and neither it nor `docs/limitations.md` names the five providers the connector catalog actually covers. Each of those needs a source-of-truth decision from a code owner rather than an edit from this file.

## Sources consulted

- `src-tauri/src/connectors.rs` (this repository) — the one flagged code ground admitted by the methodology above, cited only for the connector catalog's provider list because no grounding file enumerates it.
- `README.md`, `docs/features.md`, and `docs/limitations.md` (this repository) — Little Monkey's shipped-capability ground truth, plus `docs/autonomous-tasks.md` and `docs/messaging-devices-and-phones.md` where `README.md`'s documentation table delegates to them. `ROADMAP.md` is used for the roadmap-linkage line and, as the second flagged exception, for the named Dispatch policies and the routing-versus-Privacy-Firewall order.
...
- [docs.anythingllm.com](https://docs.anythingllm.com/), [docs.anythingllm.com/features/memories](https://docs.anythingllm.com/features/memories) and [anythingllm.com/mobile](https://anythingllm.com/mobile).
...
- [replit.com/blog — "Introducing Agent 3: Our Most Autonomous Agent Yet"](https://replit.com/blog/introducing-agent-3-our-most-autonomous-agent-yet) — published 2025-09-10; every Replit rating in this audit rests on that one announcement, which is a year older than the audit date.
...
- [learn.chatgpt.com/codex](https://learn.chatgpt.com/codex), [learn.chatgpt.com/codex/computer-use](https://learn.chatgpt.com/codex/computer-use), [learn.chatgpt.com/codex/integrated-terminal](https://learn.chatgpt.com/codex/integrated-terminal) and [learn.chatgpt.com/docs](https://learn.chatgpt.com/docs) — both OpenAI Codex rows. The computer-use and integrated-terminal pages are what separate the two rows in categories 3 and 10. `https://openai.com/codex/` returns HTTP 403 to automated fetch as of 2026-09-03 and is therefore not a source here.
